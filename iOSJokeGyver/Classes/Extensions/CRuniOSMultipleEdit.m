...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
	...
	textView.keyboardType = (UIKeyboardType)keyboard;	// use the default type input method (entire keyboard)
	textView.returnKeyType = (UIReturnKeyType)ret;
	textView.text=text;
	textView.textAlignment = (NSTextAlignment)align;
    if ((flags&MEFLAG_PASSWORD)!=0)
    {
        textView.secureTextEntry=YES;
    }
    if ((flags&MEFLAG_TVVISIBLE)!=0)
    {
        textView.hidden=NO;
    }
    else
