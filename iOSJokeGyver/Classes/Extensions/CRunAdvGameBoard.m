...
-(void)MoveBrick:(int)sourceX withSrceY:(int)sourceY andDestX:(int)destX andDestY:(int)destY
{
	...
	if (([self getPos:destX withY:destY] != -1) && ([self getPos:sourceX withY:sourceY] != -1))
	{
		...
		int sourceBrick = [self getBrick:sourceX withY:sourceY];
		int destinationBrick = [self getBrick:destX withY:destY];

		if (TriggerMoved && sourceBrick != 0)
		{
		...
		if (TriggerDeleted && destinationBrick != 0)
		{
...
-(void)fall
{
	for (int x = 0; x < BSizeX; x++)
	{
		...
		{
			if ([self getBrick:x withY:y] != 0 && [self getBrick:x withY:y + 1] == 0)
			{
...
-(void)fallLEFT
{
	for (int y = 0; y <= BSizeY; y++)
	{
		...
		{
			if ([self getBrick:x withY:y] != 0 && [self getBrick:x - 1 withY:y] == 0)
			{
...
-(void) actImportActives:(CObject*)selected
{
	...
	{
		FixedBoard[size - AddIncrement - 1] = (selected->hoCreationId * 65536) + selected->hoNumber;
	}
	AddIncrement++;
}
...
-(CObject*)CObjectFromFixed:(int)fixed
{
	...
	{
		if (((pObject->hoCreationId * 65536) + pObject->hoNumber) == fixed)
		{
			return pObject;
		}
	}
