...
-(BOOL)cndAtBackRout:(int)param1
{
	...
	if (sprPtr != nil && sprPtr->sprLayer == nLayer)
	{
		...
		int FValue = (roPtr->hoCreationId * 65536) + (((int) roPtr->hoNumber) & 0xFFFF);
...
-(BOOL)cndAboveRout:(int)param1 withParam1:(int)param2
{
	...
	for (o1 = 0; o1 < ho->hoAdRunHeader->rhNObjects; o1++)
	{
		...
		FValue1 = (roPtr1->hoCreationId * 65536) + (((int) roPtr1->hoNumber) & 0xFFFF);
		if (param1 == FValue1)
		{
			int count2 = 0;
			int o2;
			...
			for (o2 = 0; o2 < ho->hoAdRunHeader->rhNObjects; o2++)
			{
				...
				FValue2 = (roPtr2->hoCreationId * 65536) + (((int) roPtr2->hoNumber) & 0xFFFF);
...
-(void)action:(int)num withActExtension:(CActExtension*)act
{
	switch (num)
	{
		...
		case 43:
			...
			break;
        case 44:    // ACT_SETLAYEREFFECT
            [self actSetLayerEffect:act];
            break;
        case 45:    // ACT_SETLAYEREFFECTPARAM
            [self actSetLayerEffectParam:act];
            break;
        case 46:    // ACT_SETLAYERALPHA
            [self actSetLayerEffectAlpha:act];
            break;
        case 47:    // ACT_SETLAYERRGB
            [self actSetLayerEffectRGB:act];
            break;
        case 48:    // ACT_SETLAYEREFFECT_BYNAME
            [self actSetLayerEffectByName:act];
            break;
        case 49:    // ACT_SETLAYEREFFECTPARAM_BYNAME
            [self actSetLayerEffectParamByName:act];
            break;
        case 50:    // ACT_SETLAYERALPHA_BYNAME
            [self actSetLayerEffectAlphaByName:act];
            break;
        case 51:    // ACT_SETLAYERRGB_BYNAME
            [self actSetLayerEffectRGBByName:act];
            break;
        case 52:    // ACT_SETLAYEREFFECTTEXTUREPARAM
            [self actSetLayerEffectTextureParam:act];
            break;
        case 53:    // ACT_SETLAYEREFFECTTEXTUREPARAM_BYNAME
            [self actSetLayerEffectTextureParamByName:act];
            break;
	}
}
...
-(void)actSetLayerEffect:(CActExtension*)act
 {
     int param1 = [act getParamExpression:rh withNum:0];
     ...
     if (param1 > 0 && param1 <= ho->hoAdRunHeader->rhFrame->nLayers)
     {
        ...
        if (name != nil && [name length] > 0)
        {
          ...
          {
              if([pLayer checkOrCreateEffectIfNeeded:ho->hoAdRunHeader->rhApp andName:name] != -1)
              {
                  effect = BOP_EFFECTEX;
                  pLayer->effectShader = [pLayer->effectEx getIndexShader];
              }
          }
        }

        pLayer->effect &=~BOP_MASK;
        pLayer->effect |= effect;
     }
 }

 -(void)actSetLayerEffectParam:(CActExtension*)act
 {
     int param1 = [act getParamExpression:rh withNum:0];
     NSString* paramName = [act getParamExpString:rh withNum:1];
     CValue* paramValue = [ho->hoAdRunHeader get_EventExpressionAny:[act getParamEvery:rh withNum:2]];

     if (param1 > 0 && param1 <= ho->hoAdRunHeader->rhFrame->nLayers)
     {
         CLayer* pLayer = ho->hoAdRunHeader->rhFrame->layers[param1 - 1];
             
         if((pLayer->effect & BOP_MASK) == BOP_EFFECTEX)
         {
            if(pLayer->effectEx != nil)
            {
                int paramIndex = [pLayer->effectEx getParamIndex:paramName];
                if(paramIndex != -1)
                {
                    [pLayer->effectEx setParamValue:paramIndex andValue:paramValue];
                }
            }
         }
 ...
 -(void)actSetLayerEffectAlpha:(CActExtension*)act
 {
     int param1 = [act getParamExpression:rh withNum:0];
     ...
     if (param1 > 0 && param1 <= ho->hoAdRunHeader->rhFrame->nLayers)
     {
         ...
         unsigned char alpha = (unsigned char)clamp(255-[act getParamExpression:rh withNum:1], 0, 255);
         
         BOOL wasSemi = ((pLayer->effect & BOP_RGBAFILTER) == 0);
         pLayer->effect = (pLayer->effect & BOP_MASK) | BOP_RGBAFILTER;
         unsigned int rgbaCoeff = 0x00FFFFFF;
         
         if(!wasSemi)
             rgbaCoeff = pLayer->effectParam;
         
         unsigned int alphaPart = (unsigned int)alpha << 24;
         unsigned int rgbPart = (rgbaCoeff & 0x00FFFFFF);
         pLayer->effectParam = alphaPart | rgbPart;
         
     }
 }

 -(void)actSetLayerEffectRGB:(CActExtension*)act
 {
     int param1 = [act getParamExpression:rh withNum:0];
     ...
     if (param1 > 0 && param1 <= ho->hoAdRunHeader->rhFrame->nLayers)
     {
         ...
         unsigned char argb = (unsigned char)[act getParamExpression:rh withNum:1];
         BOOL wasSemi = ((pLayer->effect & BOP_RGBAFILTER) == 0);
         pLayer->effect = (pLayer->effect & BOP_MASK) | BOP_RGBAFILTER;
         
         unsigned int rgbaCoeff = pLayer->effectParam;
         unsigned int alphaPart;
         if(wasSemi)
         {
             if(pLayer->effectParam == -1)
                 alphaPart = 0xFF000000;
             else
                 alphaPart = 255-(pLayer->effectParam*2 << 24);
         }
         else
         ...
         pLayer->effectParam = filter;

     }

 }

 -(void)actSetLayerEffectByName:(CActExtension*)act
 {
     NSString* param1 = [act getParamExpString:rh withNum:0];
     NSString* name = [act getParamExpString:rh withNum:1];
 ...
 -(void)actSetLayerEffectParamByName:(CActExtension*)act
 {
     NSString* param1 = [act getParamExpString:rh withNum:0];
     NSString* paramName = [act getParamExpString:rh withNum:1];
     CValue* paramValue = [ho->hoAdRunHeader get_EventExpressionAny:[act getParamEvery:rh withNum:2]];
 ...
 -(void)actSetLayerEffectAlphaByName:(CActExtension*)act
 {
     NSString* param1 = [act getParamExpString:rh withNum:0];
 ...
 -(void)actSetLayerEffectRGBByName:(CActExtension*)act
 {
     NSString* param1 = [act getParamExpString:rh withNum:0];
 ...
 -(void)actSetLayerEffectTextureParam:(CActExtension*)act
 {

 }

 -(void)actSetLayerEffectTextureParamByName:(CActExtension*)act
 {
...
-(CValue*)expression:(int)num
{
	switch (num)
	{
        ...
        case 20:  // EXP_GETLAYEREFFECTPARAM
            return [self expGetLayerEffectParam];
        case 21: // EXP_GETLAYERALPHA
            return [self expGetLayerEffectAlpha];
        case 22: // EXP_GETLAYERRGB
            return [self expGetLayerEffectRGB];
        case 23:  // EXP_GETLAYEREFFECTPARAM_BYNAME
            return [self expGetLayerEffectParamByName];
        case 24: // EXP_GETLAYERALPHA_BYNAME
            return [self expGetLayerEffectAlphaByName];
        case 25: // EXP_GETLAYERRGB_BYNAME
            return [self expGetLayerEffectRGBByName];
	}
	return nil;
}
...
-(CValue*)expGetFV
{
	...
	for (no = 0; no < ho->hoAdRunHeader->rhNObjects; no++)
	{
		...
		{
			FValue = (roPtr->hoCreationId * 65536) + (((int) roPtr->hoNumber) & 0xFFFF);
...
-(CValue*)expGetTopFV
{
	...
	while (sprPtr != nil)
	{
		...
		if (sprPtr->sprLayer == nLayer && (sprPtr->sprFlags & SF_TOKILL) == 0)
		{
			...
			return [rh getTempValue:(roPtr->hoCreationId * 65536) + (((int) roPtr->hoNumber) & 0xFFFF)];
		}
		sprPtr = sprPtr->objPrev;
	}
	return [rh getTempValue:0];
...
-(CValue*)expGetLayerEffectParam {
    int nLayer = [[ho getExpParam] getInt];
    NSString* paramName = [[ho getExpParam] getString];
    ...
    if (nLayer > 0 && nLayer <= ho->hoAdRunHeader->rhFrame->nLayers)
    {
        ...
        if((pLayer->effect & BOP_MASK) == BOP_EFFECTEX) {
            if (pLayer->effectEx != nil) {
                int index = [pLayer->effectEx getParamIndex:paramName];
                if (index != -1) {
                    switch ([pLayer->effectEx getParamType:index]) {
                        case EFFECTPARAM_INTFLOAT4:
                        case EFFECTPARAM_INT:
                            return [rh getTempValue:[pLayer->effectEx getParamInt:index]];
                        case EFFECTPARAM_FLOAT:
                            return [rh getTempValue:[pLayer->effectEx getParamFloat:index]];
                    }
                }
            }
        }
...
-(CValue*)expGetLayerEffectAlpha {
    int nLayer = [[ho getExpParam] getInt];

    if (nLayer > 0 && nLayer <= ho->hoAdRunHeader->rhFrame->nLayers)
    {
        CLayer* pLayer = ho->hoAdRunHeader->rhFrame->layers[nLayer - 1];
        int effect = pLayer->effect;
        int effectParam = pLayer->effectParam;
        int alpha = 0;
        ...
        return [rh getTempValue:alpha];
    }
    return [rh getTempValue:0];
}

-(CValue*)expGetLayerEffectRGB {
    int nLayer = [[ho getExpParam] getInt];
    ...
    if (nLayer > 0 && nLayer <= ho->hoAdRunHeader->rhFrame->nLayers)
    {
        ...
        return [rh getTempValue:argb];

    }
   return [rh getTempValue:0];
}

-(CValue*)expGetLayerEffectParamByName {
    NSString* pName = [[ho getExpParam] getString];
    NSString* paramName = [[ho getExpParam] getString];
...
-(CValue*)expGetLayerEffectAlphaByName {
    NSString* pName = [[ho getExpParam] getString];
...
-(CValue*)expGetLayerEffectRGBByName {
    NSString* pName = [[ho getExpParam] getString];
...
//	CObject* roPtr;
	...
	for (no = 0; no < ho->hoAdRunHeader->rhNObjects; no++)
	{
		...
		count++;
		fValue = (hoPtr->hoCreationId * 65536) + hoPtr->hoNumber;
...
-(CObject*)lyrGetROfromFV:(int)fixedValue
{
	...
	for (no = 0; no < ho->hoAdRunHeader->rhNObjects; no++)
	{
		...
		CObject* hoPtr = ho->hoAdRunHeader->rhObjectList[count];
		count++;
		fValue = (hoPtr->hoCreationId * 65536) + (((int) hoPtr->hoNumber) & 0xFFFF);
		if (fixedValue == fValue)
		{
			return hoPtr;
		}
	...
	while (pSprite != nil)
	{
		...
		if ((pSprite->sprFlags & SF_TOKILL) == 0)
		{
			...
			if(hoPtr != nil)
			{
				...
				if (hoPtr->rov != nil && altValue < hoPtr->rov->rvNumberOfValues)
				{
...
-(int)lyrGetFVfromEVP:(LPEVP)evp
{
	...
	return ((hoPtr->hoCreationId * 65536) + (((int) hoPtr->hoNumber) & 0xFFFF));
}
		...
		if (param2 != nil)
		{
			...
			for (i = 1; i <= oilPtr1->oilNObjects; i++)
			{
				...
				if (roPtr1->hoNumNext > -1)
				{
					...
					FValue1 = (roPtr1->hoCreationId * 65536) + roPtr1->hoNumber;
				}
			}
		}
		else
