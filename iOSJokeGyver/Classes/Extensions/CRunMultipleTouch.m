...
-(BOOL)touchBegan:(UITouch*)touch
{
	...
	if (n<MAX_TOUCHES && touches[n].touche==nil)
	{
        ...
        int touchX = (x) - rhApp->parentX;
        int touchY = (y) - rhApp->parentY;
...
-(void)touchMoved:(UITouch*)touch
{
	...
	for (n=0; n<MAX_TOUCHES; n++)
	{
		if (touches[n].touche==touch)
		{
            ...
            int x=(int)position.x;
            int y=(int)position.y;
			touches[n].x=x - ho->hoAdRunHeader->rhApp->parentX;
			touches[n].y=y - ho->hoAdRunHeader->rhApp->parentY;
			touches[n].dragX=x;
			touches[n].dragY=y;
			lastTouch=n;
...
-(void)callObjectConditions:(int)x withY:(int)y
{
    ...
    CObject* pHox;
    int count=0;
    for (int nObject = 0; nObject < rh->rhNObjects; nObject++)
    {
    ...
    for (count=0; count<[list size]; count++)
    {
        ...
        OiUnder=pHox->hoOi;
        hoUnder=(pHox->hoCreationId * 65536) | (pHox->hoNumber & 0xFFFF);
...
- (BOOL)selectObjectByFixedValue:(short)oiList withHoFV:(int)hoFV
{
    ...
    do
    {
        ...
        do
        {
            int fv = (pHo->hoCreationId * 65536) | (pHo->hoNumber & 0xFFFF);
            if (fv != hoFV)
            {
                count--;
                [evtProg evt_DeleteCurrentObject];
...
-(BOOL)isObjectUnder:(CObject*)pHox withX:(int)x andY:(int)y
{
	if ((pHox->hoFlags&HOF_DESTROYED)==0)
	{
		...
		{
            //return YES;  change in favor of extension size
            //Added Image Hot Spot, some exts, as APO does require it.
            return (x >= pHox->hoX-pHox->hoImgXSpot && y >= pHox->hoY-pHox->hoImgYSpot && x < pHox->hoX+pHox->hoImgWidth-pHox->hoImgXSpot && y < pHox->hoY+pHox->hoImgHeight-pHox->hoImgYSpot);
        }
	}
    return NO;
}
...
-(BOOL)cndNewTouchObject:(CCndExtension*)cnd
{
    if ((ho->hoFlags & HOF_TRUEEVENT) != 0)
    {
        ...
        if ( OiUnder != pParam->evp.evpW.evpW1 && pParam->evp.evpW.evpW1 >= 0 )
			return false;
...
-(CValue*)expGetX
{
	...
	if (touch>=0 && touch<MAX_TOUCHES)
	{
        int finalX = touches[touch].x - rh->rhApp->parentX + rh->rhWindowX;
        if(rh->rhApp->subApp != nil && rh->rhApp->parentApp != nil)
            finalX -= rh->rhApp->parentApp->run->rhWindowX;
		return [rh getTempValue:finalX];
	}
...
-(CValue*)expGetY
{
	...
	if (touch>=0 && touch<MAX_TOUCHES)
	{
        int finalY = touches[touch].y - rh->rhApp->parentY + rh->rhWindowY;
        if(rh->rhApp->subApp != nil)
             finalY -= rh->rhApp->parentApp->run->rhWindowY;
        return [rh getTempValue:finalY];
	}
	return [rh getTempValue:-1];
}
...
-(CArrayList*)Resample:(CArrayList*)points withNum:(int)n
{
	...
	while ([newpoints size]<n) // sometimes we fall a rounding-error short of adding the last point, so add it if so
	{
