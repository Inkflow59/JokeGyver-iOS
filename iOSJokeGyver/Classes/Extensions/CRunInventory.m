...
-(void)SetSlider
{
    if ((flags&IFLAG_HSCROLL)!=0)
    {
        ...
        [slider Initialise:rh withParam1:x andParam2:y andParam3:ho->hoImgWidth andParam4:SY_SLIDER andParam5:scrollColor andParam6:scrollColor2 andParam7:self];
    }
    else if ((flags&IFLAG_VSCROLL)!=0)
    {
        int x=ho->hoX+ho->hoImgWidth-SX_SLIDER;
        int y=ho->hoY;
        [slider Initialise:rh withParam1:x andParam2:y andParam3:SX_SLIDER andParam4:ho->hoImgHeight andParam5:scrollColor andParam6:scrollColor2 andParam7:self];
    }
}
...
{
    return (pho->hoCreationId*65536)|(pho->hoNumber&0xFFFF);
}
...
-(void)Scroll:(int)typeT withSlide:(int)slide
	{
		int pos=position;
		...
		{
			position=pos;
            [slider SetPosition:position withParam1:MIN([displayList size]-position, nLines*nColumns) andParam2:[displayList size]];
			bRedraw=true;
			bUpdateList=true;
		}
	}
    ...
    if (type == INVTYPE_LIST)
    {
        ...
        if ([displayList size]>0)
        {
            ...
            if ((flags&IFLAG_MOUSE)!=0)
            {
                ...
                {
					if ([slider IsMouseInBar:xx withYY:yy] == NO)
					{
						x/=itemSx;
						y/=itemSy;
            ...
            if (bMouse && slider->bInitialised == YES)
            {
				if ([slider IsMouseInBar:xx withYY:yy] == NO)
				{
...
-(int)Save:(char*)ptr
{
    ...
    size+=WriteAnInt(ptr+size, [list size]);
    for (n=0; n<[list size]; n++)
    {
        pItem=(CInventoryItem*)[list get:n];
...
-(void)Initialise:(CRun*)rh withParam1:(int)x andParam2:(int)y andParam3:(int)sx andParam4:(int)sy andParam5:(int)c andParam6:(int)ch andParam7:(CRunInventory*)d
{
    rhPtr=rh;
    color=c;
    colorHilight=ch;
...
-(BOOL)IsMouseInBar:(int)xx withYY:(int)yy
	{
		if (bInitialised)
		{
			return surface.containsPoint(xx, yy);
		}
		return NO;
	}
-(BOOL)IsDragging
...
-(int)GetZone:(int)xx withYY:(int)yy
	{
		if (bDragging)
		{
			return ZONE_SLIDER;
		}
		if (topArrow.containsPoint(xx, yy))
		{
			return ZONE_TOPARROW;
		}
		if (bottomArrow.containsPoint(xx, yy))
		{
			return ZONE_BOTTOMARROW;
		}
		if (center.containsPoint(xx, yy))
		{
			if (slider.containsPoint(xx, yy))
			{
				return ZONE_SLIDER;
			}
			if (bHorizontal)
...
-(void) Handle:(int)xx withYY:(int)yy
	{
		int delta;
		int pos;
		if (bInitialised && length<total)
		{
            BOOL bDown=rhPtr->rhApp->bMouseDown;
            if (bDown!=oldBDown)
			{
				oldBDown=bDown;
				zone=[self GetZone:xx withYY:yy];
				...
				if (bDown)
				{
					...
					{
						case ZONE_TOPARROW:
                            [data Scroll:SCROLL_UP withSlide:0];
							break;
						case ZONE_TOPCENTER:
                            [data Scroll:SCROLL_PAGEUP withSlide:0];
							break;
						case ZONE_BOTTOMCENTER:
                            [data Scroll:SCROLL_PAGEDOWN withSlide:0];
							break;
						case ZONE_BOTTOMARROW:
                            [data Scroll:SCROLL_DOWN withSlide:0];
							break;
						case ZONE_SLIDER:
							xStart=xx;
							yStart=yy;
				...
				if (bDragging)
				{
					if (bHorizontal)
					{
						...
						if (delta!=0)
						{
							pos=position+delta;
							pos=MAX(pos, 0);
							pos=MIN(pos, total-length);
                            [data Scroll:SCROLL_SLIDE withSlide:pos];
							xStart=xx;
						}
					}
					else
...
-(void)DrawBar:(CRenderer*)renderer
{
    if (bInitialised==true&& length<total)
    {
        ...
        invFillRect(renderer, slider, sColor);
		...
		if (bHorizontal)
		{
			...
			invFillRect(renderer, topArrow, lcolor);
			...
			invFillRect(renderer, bottomArrow, lcolor);
		}
		else
		{
			if (zone==ZONE_TOPARROW)
