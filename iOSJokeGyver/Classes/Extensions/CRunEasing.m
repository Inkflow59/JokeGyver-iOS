...
-(void)pauseRunObject
{
	[self pauseAllObjects];
}

-(void)continueRunObject
{
	[self resumeAllObjects];
}
...
BOOL filterPaused(CObject* rdPtr, CObject* object)
{
	int fixed = [object fixedValue];
	CRunEasing* easing = (CRunEasing*)rdPtr;
	for( int i=0; i< [easing->controlled size]; ++i)
	{
		MoveStruct* moved = (MoveStruct*)[easing->controlled get:i];
		if(moved->mobject == fixed && moved->pausecount != 0)
			return YES;
	}
	return NO;
}
...
-(BOOL)condition:(int)num withCndExtension:(CCndExtension*)cnd
{
	switch (num)
	{
		...
		case CND_ISOBJECTPAUSED:
		{
			...
			BOOL ret = [rh->objectSelection filterObjects:(CObject*)self andOi:oi andNegate:isNegated andFilterFunction:&filterPaused];
			return ret;
		}
	}
	return NO;
...
-(void)action:(int)num withActExtension:(CActExtension*)act
{
	switch (num)
	{
		...
		case ACT_PAUSEOBJECT:
			[self pauseObject:[act getParamObject:rh withNum:0]];
			break;
		case ACT_PAUSEALLOBJECTS:
			[self pauseAllObjects];
			break;
		case ACT_RESUMEOBJECT:
			[self resumeObject:[act getParamObject:rh withNum:0]];
			break;
		case ACT_RESUMEALLOBJECTS:
			[self resumeAllObjects];
			break;
	}
}
...
-(void)pauseObject:(CObject*)object
{
	int fixed = [object fixedValue];
	for(int i = 0; i < [controlled size]; i++)
	{
		...
		if(moved->mobject == fixed)
		{
			if (moved->pausecount == 0 && moved->timeMode == 0)
				moved->pausetime = [NSDate timeIntervalSinceReferenceDate];
			moved->pausecount++;
			return;
		}
	}
...
-(void)resumeObject:(CObject*)object
{
	int fixed = [object fixedValue];
	for(int i = 0; i < [controlled size]; i++)
	{
		...
		if(moved->mobject == fixed)
		{
			if (moved->pausecount != 0)
			{
				moved->pausecount--;
				if (moved->pausecount == 0 && moved->timeMode == 0)
					moved->starttime += [NSDate timeIntervalSinceReferenceDate] - moved->pausetime;
			}
			return;
		}
	}
