...
@interface CRunkcinput () <ModalInputDelegate>
@end
...
{
    if([alert.actions indexOfObject:action] == 0)
    {
        [string release];
        string = [[NSString alloc] initWithString:modalInput.text];
...
{
	[rh pause];
    MainViewController* mainView = ho->hoAdRunHeader->rhApp->mainViewController;
	if(modalInput != nil)
		[modalInput release];
    modalInput = [[ModalInput alloc] initStringWithTitle:title message:description fromViewController:mainView cancelButtonTitle:@"Cancel" okButtonTitle:@"OK"];
    modalInput.delegate = self;
    [modalInput setTextField:string at:0];
	[modalInput show];
}
...
-(void)act_inputNumber:(NSString*)title description:(NSString*)description
{
	...
	if(modalInput != nil)
		[modalInput release];
    modalInput = [[ModalInput alloc] initNumberWithTitle:title message:description fromViewController:mainView cancelButtonTitle:@"Cancel" okButtonTitle:@"OK"];
    modalInput.delegate = self;
    [modalInput setTextField:string at:0];
	[modalInput show];
}
...
-(void)act_inputUsernamePassword:(NSString*)title description:(NSString*)description
{
	...
	if(modalInput != nil)
		[modalInput release];
	modalInput = [[ModalInput alloc] initNamePasswordWithTitle:title message:description fromViewController:mainView cancelButtonTitle:@"Cancel" okButtonTitle:@"OK"];
    modalInput.delegate = self;
    [modalInput setTextField:string at:0];
    [modalInput setTextField:string at:1];
	[modalInput show];
}
...
-(void)act_inputMultiline:(NSString*)title description:(NSString*)description
{
	...
	if(modalInput != nil)
		[modalInput release];
	modalInput = [[ModalInput alloc] initMultiLineStringWithTitle:title message:description fromViewController:mainView cancelButtonTitle:@"Cancel" okButtonTitle:@"OK"];
    modalInput.delegate = self;
    [modalInput setTextField:string at:0];
	[modalInput show];
}
