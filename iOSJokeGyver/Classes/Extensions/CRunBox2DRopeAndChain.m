...
void rcrRemoveObject(void* ptr, void* movement);
...
short rCreateRunObject(LPRDATARC rdPtr, CFile* file)
{
	...
	rdPtr->pRemoveObject = rcrRemoveObject;
    ...
    rdPtr->effectShader= rdPtr->ho->ros->rsEffectShader;
...
short rHandleRunObject(LPRDATARC rdPtr)
{
    ...
    if(rdPtr->base->bPaused)
    {
        ...
        if (rdPtr->ho->ros->rsEffect != rdPtr->effect
            || rdPtr->ho->ros->rsEffectParam != rdPtr->effectParam
            || rdPtr->ho->ros->rsEffectShader != rdPtr->effectShader)
        {
            rdPtr->effect = rdPtr->ho->ros->rsEffect;
            rdPtr->effectParam = rdPtr->ho->ros->rsEffectParam;
            rdPtr->effectShader= rdPtr->ho->ros->rsEffectShader;
            ...
            {
                element = (CElement*)rdPtr->elements->Get(n);
                element->setEffect(rdPtr->effect, rdPtr->effectParam, rdPtr->effectShader);
            }
        }
    ...
    else
    {
        ...
        if (rdPtr->elements->Size() == 0)
        {
            ...
            float angle = - (float)(rdPtr->angle * b2_pi / 180.0f );
            ...
            element->m_mBase->m_body = rdPtr->base->pCreateBody(rdPtr->base, b2_dynamicBody, x, y, rdPtr->angle, rdPtr->gravity, (void*)element->m_mBase, 0, 0);
...
int REXP_GETX1(LPRDATARC rdPtr)
{
    ...
    if (element)
    {
        ...
        int x, y;
        float angle;
        rdPtr->base->pGetBodyPosition(rdPtr->base, element->m_mBase->m_body, &x, &y, &angle);
        return x;
    }
	return 0;
}
...
void CElement::setPosition()
{
	...
	CRSpr* rsPtr=parent->ho->ros;
	if(nLayer != rsPtr->rsLayer)
	{
		nLayer = rsPtr->rsLayer;
		[rhPtr->spriteGen setSpriteLayer:sprite withLayer:nLayer];
	}
}
void CElement::setEffect(int effect, int effectParam, int effectShader)
{
	[parent->rh->spriteGen modifSpriteEffect:sprite withInkEffect:effect andInkEffectParam:effectParam andEffectShader:effectShader];
}
...
{
    m_object = calloc(sizeof(RUNDATARC), 1);
