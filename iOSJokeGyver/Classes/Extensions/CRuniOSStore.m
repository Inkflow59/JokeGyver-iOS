...
- (void)sendEvents
{
    if (storeListeners.count > 0)
    {
        ...
        NSLog(@"Sending events from store ...");
        
        if (requestResponses.count > 0 || invalidProductIdentifiers > (void *)0)
        {
            @synchronized(storeObject->ho)
            {
...
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error
{
    ...
    if (storeListeners.count > 0 )
    {
        ...
        {
            [storeObject->ho resume];
            [storeObject->ho pushEvent:CND_RESTOREPAYMENTSFAILED withParam:0];
        }
    }
}
...
{
    NSLog(@"Removed transactions from queue");
}
...
{
    NSLog(@"Should Add Store Payment ...");
    BOOL bRet = NO;
    if (storeListeners.count > 0 )
    {
        CRuniOSStore *storeObject = [storeListeners objectAtIndex:0];
        bRet = [storeObject shouldAddStore:(NSString*)payment.productIdentifier];
    }
    return bRet;
}
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error
{
    ...
    if (storeListeners.count > 0)
    {
        ...
        @synchronized(storeObject->ho)
        {
            ...
            //NSLog(@"resume from failed with error ...");
            [storeObject->ho resume];
...
{
    payAvailable = [SKPaymentQueue canMakePayments];
    if([NSThread isMainThread])
    {
        for (NSString *invId in invalidIdentifiers)
        {
            ...
            {
                NSLog(@"Invalid product identifier (returned nil) - Unknown error.");
            }
        }
        ...
        for (SKProduct *product in products)
        {
            ...
            [formatter setLocale:product.priceLocale != nil ? product.priceLocale : [NSLocale currentLocale]];
        ...
        switch (transaction.transactionState)
        {
            ...
            case SKPaymentTransactionStatePurchased:
            {
                ...
                if (floor(NSFoundationVersionNumber) >= NSFoundationVersionNumber_iOS_7_1)
                {
                    ...
                    if(dataReceipt != nil)
                        storeReceipt = [[NSString alloc] initWithString:[dataReceipt base64EncodedStringWithOptions:0]];
                    else
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                    storeReceipt = [[NSString alloc] initWithString:[transaction.transactionReceipt base64encode]];
#pragma clang diagnostic pop
                }
                //NSLog(@"resume from updated purchased transactions ...");
                if([ho isPaused])
                    [ho resume];
            ...
            case SKPaymentTransactionStateFailed:
            {
                ...
                //NSLog(@"resume from failed transactions ...");
                if([ho isPaused])
                    [ho resume];
                if([ho isPaused])
                    [ho resume];
...
#pragma clang diagnostic pop
                ...
                //NSLog(@"resume from restored transactions ...");
                if([ho isPaused])
                    [ho resume];
...
-(BOOL)shouldAddStore:(NSString *)productId
{
    if([productsToRequest containsObject:productId])
        return YES;
    return NO;
}
...
-(int)handleRunObject
{
    if(productsToRequest.count > 0)
    {
        ...
        SKProductsRequest* request = [[[SKProductsRequest alloc] initWithProductIdentifiers:[NSSet setWithArray:productsToRequest]] retain];
...
-(void)act_RequestPaymentSlot:(NSString*)slot andQuantity:(int)quantity
{
    ...
    //NSLog(@"paused from action transactions ...");
    [ho pause];
...
-(void)act_RestoreTransactions
{
    ...
    //NSLog(@"paused from action restore ...");
    [ho pause];
