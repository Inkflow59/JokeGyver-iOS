...
short b2fCreateRunObject(LPRDATAF rdPtr, CFile* file)
{
	...
	rdPtr->strength=(float)(((float)rdPtr->strengthBase)/100.0f/5.0f);
	rdPtr->base=nil;
...
short b2fHandleRunObject(LPRDATAF rdPtr)
{
	if (!b2fStartObject(rdPtr) || rdPtr->base->bPaused)
		return 0;
...
void b2fRACT_SETSTRENGTH(LPRDATAF rdPtr, int param1, int param2)
{
	rdPtr->strength=(float)(((float)param1)/100.0f/5.0f);
...
{
    m_object = calloc(sizeof(RUNDATAF), 1);
