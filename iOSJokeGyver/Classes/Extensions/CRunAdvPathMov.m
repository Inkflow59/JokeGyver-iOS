...
-(void)action:(int)num withActExtension:(CActExtension*)act
{
	switch (num)
	{
		...
		case AID_Findjourney:
			...
			break;
		case AID_LoadPath:
			[self LoadPath:[act getParamExpString:rh withNum:0]];
			break;
		case AID_SavePath:
            [self SavePath:[act getParamExpString:rh withNum:0]];
			break;
...
- (void)LoadPath:(NSString *)fileName
{
    NSData *myData = nil;
    CFile *file = nil;
    CRunApp *app = ho->hoAdRunHeader->rhApp;
    myData = [app loadResourceData:fileName];
    ...
    file = [[CFile alloc] initWithNSData:myData];

    long file_size = (long)[myData length];
    ...
    while (!finishedloading)
    {
        ...
        if ((!finishedloading) && ((Currentpos > 1) && ((Currentpos % 2) == 1)))
        {
            NSData* data = [file readNSData:sizeof(float)];
            [data getBytes:&Loadfloat length:sizeof(float)];
            file_size -= 4;
       }
        if ((special > 0) && (!finishedloading))
        {
            if (Currentpos == 0)
            {
                [mypointer->myvector add:[[AdvPathMovPoints alloc] initWithParam1:Loadnumber andParam2:0]];
            }
            if (Currentpos == 1)
            {
            ...
            { //MessageBox(NULL, "Loading Point ID", NULL,MB_OK );
                mypointer->theIterator = (AdvPathMovPoints *)[mypointer->myvector get:vectorpos];
                [mypointer->theIterator->Connections add:[[AdvPathMovConnect alloc] initWithParam1:Loadnumber andParam2:5]];
            }
...
-(NSData *) dataFromInt:(int)num {
    NSData * data = [NSData dataWithBytes:&num length:sizeof(int32)];
    return data;
}

-(NSData *) dataFromFloat:(float)num {
    NSData * data = [NSData dataWithBytes:&num length:sizeof(float)];
    return data;
}

- (void)SavePath:(NSString *)fileName
{
    int i;
    int j;

    NSMutableData *myData = [NSMutableData alloc];

    for (i = 0; i < [mypointer->myvector size]; i++)
    {
        mypointer->theIterator = (AdvPathMovPoints *)[mypointer->myvector get:i];
        [myData appendData:[self dataFromInt:([mypointer->theIterator->Connections size] * 2 + 2)]];
        [myData appendData:[self dataFromInt:mypointer->theIterator->X]];
        [myData appendData:[self dataFromInt:mypointer->theIterator->Y]];

        for (j = 0; j < [mypointer->theIterator->Connections size]; j++)
        {
            mypointer->theIterator->ConnectIterator = (AdvPathMovConnect *)[mypointer->theIterator->Connections get:j];
            [myData appendData:[self dataFromInt:mypointer->theIterator->ConnectIterator->PointID]];
            [myData appendData:[self dataFromFloat:mypointer->theIterator->ConnectIterator->Distance]];
        }
    }
    ...
    if ([myData length] > 0)
    {
        if(![myData writeToFile:path atomically:NO])
            NSLog(@"Path file write to path: %@", path);
    }
    [myData release];
}
