...
-(void)mouseClicked
{
	...
	if (pData != nil)
	{
		...
		{
			if (rNumInObjList == [pData GetObjectFromList:rh->rh2MouseX+rh->rhWindowX  withParam1:rh->rh2MouseY+rh->rhWindowY])
			{
...
-(void)DisplayObject:(CRenderer*)renderer withParam1:(CRunApp*)idApp andParam2:(CRect)rc andParam3:(KcBoxACData*)pc andParam4:(NSString*)text andParam5:(CFontInfo*)hFnt andParam6:(KcBoxACData1*)pdata1
{
	...
	if ((pc->wImage != nil) && ((pc->dwFlags & FLAG_HIDEIMAGE) == 0))
	{
		...
		if (bDisplayImage == YES)
		{
			...
			if (wc > 0 && hc > 0)
			{
                BOOL scissorFlag = renderer->usesScissor;
				renderer->setClip(xc-rh->rhWindowX, yc-rh->rhWindowY, wc, hc);
				...
				renderer->resetClip();
                renderer->usesScissor = scissorFlag;
            }
	...
	if (([text length] != 0) && (pc->textColor != COLOR_NONE))
	{
		...
		if(updated)
		{
            ...
            if ( pc->dwFlags & ALIGN_ENDELLIPSIS )
                dtFlags |= DT_ENDELLIPSIS;
            if ( pc->dwFlags & ALIGN_PATHELLIPSIS )
                dtFlags |= DT_PATHELLIPSIS;
