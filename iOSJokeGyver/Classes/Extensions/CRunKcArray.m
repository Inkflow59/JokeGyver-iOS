            ...
            for (y=0; y<pArray->lDimensionY; y++)
            {
                for (x=0; x<pArray->lDimensionX; x++)
                {
                    ...
                    if (s!=nil)
                    {
                        ...
                        length+=[s length];
#else
                        NSData *utfData = [s dataUsingEncoding:NSUTF16LittleEndianStringEncoding];
...
#else
    strcpy(pHeader->identifier, "MFU ARRAY");
#endif
    pHeader->version=2;
    pHeader->revision=0;
...
#else
                        NSData *utfData = [s dataUsingEncoding:NSUTF16LittleEndianStringEncoding];
                        pointer = [utfData length];
                        memcpy((ptr+4), [utfData bytes], pointer);
                        l = [s length];
#endif
                        *(ptr++)=l&0xFF;
                        *(ptr++)=(l>>8)&0xFF;
