...
#define ACT_INSERTTEXT 36
#define ACT_SETCARETPOS 37
#define ACT_LAST 38
...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
    ...
    if ((flags&EDIT_MULTILINE)==0)
    {
		...
		if (flags&EDIT_3DLOOK)
			border=2;
        textField.borderStyle = (UITextBorderStyle)border;
        ...
        textField.autocorrectionType = (UITextAutocorrectionType)0;	// no auto correction support
        textField.keyboardType = (UIKeyboardType)0;	// use the default type input method (entire keyboard)
        textField.returnKeyType = (UIReturnKeyType)0;
        textField.clearButtonMode = (UITextFieldViewMode)0;	// has a clear 'x' button to the right
        textField.text=@"";

        if(flags&EDIT_ALIGN_RIGHT)
            textField.textAlignment = NSTextAlignmentRight;
        if(flags&EDIT_ALIGN_HCENTER)
    ...
    else
    {
        ...
        textView.keyboardType = (UIKeyboardType)0;	// use the default type input method (entire keyboard)
        textView.returnKeyType = (UIReturnKeyType)0;
        textView.text=@"";	 
...
- (void)inputKeyboardWillShow:(NSNotification *)notification
{
    ...
    if (rh->rhApp->actualOrientation == UIInterfaceOrientationPortrait)
        newKeyboardHeight = deviceHeight - keyboardRect.origin.y;
    else if (rh->rhApp->actualOrientation == UIInterfaceOrientationPortraitUpsideDown)
        newKeyboardHeight = keyboardRect.size.height + keyboardRect.origin.y;
    else if (rh->rhApp->actualOrientation == UIInterfaceOrientationLandscapeLeft)
        newKeyboardHeight = deviceWidth - keyboardRect.origin.x;
    else
    newKeyboardHeight = keyboardRect.size.width + keyboardRect.origin.x;
    
    if (newKeyboardHeight <= 160.0f)
    {
        bhasKeyboard = YES;
    }
}
...
{
	{
		...
		gotoPosition=0;
		bGoto=YES;	
        UITextRange *selectedTextRange = tf.selectedTextRange;
        caretPosition = (int)[tf offsetFromPosition:tf.beginningOfDocument toPosition:selectedTextRange.start];
	}
}

- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;   // return NO to not change text
{
    //NSLog(@"Word that is currently being edited is : %@", string);
    bModified=YES;
    return YES;
}
...
- (void)textViewDidEndEditing:(UITextView *)tv
{
	...
	bModified=YES;
    bEditing=NO;
    UITextRange *selectedTextRange = tv.selectedTextRange;
    caretPosition = (int)[tv offsetFromPosition:tv.beginningOfDocument toPosition:selectedTextRange.start];
...
-(void)actSETPOSITION:(CActExtension*)act
{
    ...
    ho->hoX=POSX(pos)-rh->rhWindowX;
    ho->hoY=POSY(pos)-rh->rhWindowY;
    [ho redraw];
}
...
-(void)actActivate
{
    if (textView!=nil && [textView isEditable])
    {
...
-(void)actInsertText:(CActExtension*)act
{
    NSString* textToAdd = [act getParamExpString:rh withNum:0];
    int posx = [act getParamExpression:rh withNum:1];
    if([textToAdd length] > 0) {
        if (textField!=nil)
        {
            if(posx < [textField.text length])
            {
                NSUInteger currentLength = [textField.text length];
                NSString* pretext = [textField.text substringWithRange:NSMakeRange(0, posx)];
                NSString* posttext = [textField.text substringWithRange:NSMakeRange(posx+1, currentLength)];
                NSString* text = [NSString stringWithFormat:@"%@%@%@", pretext, textToAdd, posttext];
                textField.text = text;
            }
        }
        else if (textView != nil)
        {
            if(posx < [textView.text length])
            {
                NSUInteger currentLength = [textView.text length];
                NSString* pretext = [textView.text substringWithRange:NSMakeRange(0, posx)];
                NSString* posttext = [textView.text substringWithRange:NSMakeRange(posx+1, currentLength)];
                NSString* text = [NSString stringWithFormat:@"%@%@%@", pretext, textToAdd, posttext];
...
-(void)actSetCaretPosition:(CActExtension*)act
{
    int posx = [act getParamExpression:rh withNum:0];
    if (textField!=nil && textField.hidden == NO)
    {
        if(posx >= 0  && posx < [textField.text length])
        {
            NSRange range = NSMakeRange(posx, posx);
        ...
        caretPosition = (int)[textField offsetFromPosition:textField.beginningOfDocument toPosition:selectedTextRange.start];

    }
    else if(textView != nil && textView.hidden == NO)
    {
        if(posx >=0  && posx < [textView.text length])
        {
            NSRange range = NSMakeRange(posx, posx);
            UITextPosition *beginning = textView.beginningOfDocument;
            UITextPosition *start = [textView positionFromPosition:beginning offset:range.location];
            UITextPosition *end = [textView positionFromPosition:start offset:range.length];
            textView.selectedTextRange = [textView textRangeFromPosition:start
                                                              toPosition:end];
        }
...
-(void)action:(int)num withActExtension:(CActExtension*)act
{
	...
	{        
        ...
        case ACT_SETBKDCOLOR:
            ...
            break;
        case ACT_INSERTTEXT:
            [self actInsertText:act];
            break;
        case ACT_SETCARETPOS:
            [self actSetCaretPosition:act];
            break;
    }
}
...
-(CValue*)expression:(int)num
{
    switch (num)
    {
        ...
        case EXP_GETCARETPOS:
            return [self expGetCaretPosition];
    }
    return [rh getTempValue:0];
}
