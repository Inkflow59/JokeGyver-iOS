...
void createParticules(LPRDATAPARTICULES rdPtr, int number)
{
    ...
    for (n = 0; n < number; n++)
    {
		...
		particule->setEffect(rdPtr->effect, rdPtr->effectParam, rdPtr->effectShader);
        ...
        particule->fixture = rdPtr->base->pBodyCreateCircleFixture(rdPtr->base, particule->m_mBase->m_body, particule->m_mBase, x, y, (pImage->width + pImage->height + 1) / 4, rdPtr->density, rdPtr->friction, rdPtr->restitution);
...
short pHandleRunObject(LPRDATAPARTICULES rdPtr)
{
	if (!prStartObject(rdPtr) || rdPtr->base->bPaused)
		return 0;
...
void CParticule::setEffect(int effect, int effectParam, int effectShader)
{
	LPRDATAPARTICULES rdPtr = (LPRDATAPARTICULES)parent;
	[rdPtr->rh->spriteGen modifSpriteEffect:sprite withInkEffect:effect andInkEffectParam:effectParam andEffectShader:effectShader];
}
...
void CParticule::animate()
{
	...
	if (!stopped)
    {
		if ( (rdPtr->ho->hoAdRunHeader->rhFrame->leFlags & LEF_TIMEDMVTS) != 0 )
    ...
    else
    {
        if (width != oldWidth || height != oldHeight)
        {
            ...
            fixture = rdPtr->base->pBodyCreateCircleFixture(rdPtr->base, m_mBase->m_body, m_mBase, x, y, (int)((width + height + 1) / 4), rdPtr->density, rdPtr->friction, rdPtr->restitution);
        }
    }
...
{
    m_object = calloc(sizeof(RUNDATAPARTICULES), 1);
