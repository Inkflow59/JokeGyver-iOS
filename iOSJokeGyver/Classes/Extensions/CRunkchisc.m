...
@interface CRunkchisc () <ModalInputDelegate>
@end
...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
    ...
    iniFlags = ((Flags & 0x0020) != 0 ? INI_UTF8 : 0);
    ini = [CIni getINIforFile:IniName andFlags:iniFlags];
	for (int a = 0; a < 20; a++)
	{
		// Get name
	...
	else
	{
		...
		if (0 != (Flags & SCR_NAMEFIRST))
		{
			...
			for (int a = 0; a < NbScores; a++)
			{
				...
				CGSize scoreSize = [score sizeWithFSFont:thefont];
		...
		else
		{
			...
			for (int a = 0; a < NbScores; a++)
			{
				...
				CGSize nameSize = [name sizeWithFSFont:thefont];
...
-(void)endedWithAction:(UIAlertAction*)action andAlertController:(UIAlertController *)alert 
{
    ...
    {
        [self InsertNewScore:recordedScore andName:[modalInput text]];
        updated = true;
    }
}
...
-(BOOL)CheckScore:(int)player //needed public and returns true when popup is shown
{
	...
	if (player < rhPtr->rhNPlayers)
	{
		...
		if (score > Scores[NbScores - 1])
		{
            ...
            modalInput = [[ModalInput alloc] initStringWithTitle:title message:@"Enter your name:" fromViewController:ho->hoAdRunHeader->rhApp->mainViewController cancelButtonTitle:@"Cancel" okButtonTitle:@"Save"];
