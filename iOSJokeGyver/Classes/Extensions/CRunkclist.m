...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
    ...
    listFontBack = swapRGB([file readAColor]);
    flags = [file readAInt];
...
-(void)createListIfNessecary
{
    ...
    {
#ifdef __IPHONE_9_0
        pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(ho->hoX - rh->rhWindowX, ho->hoY - rh->rhWindowY, ho->hoImgWidth, ho->hoImgHeight)];
#else
        pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(ho->hoX - rh->rhWindowX, ho->hoY - rh->rhWindowY, ho->hoImgWidth, 0)];
#endif
        pickerView.backgroundColor = listBackColor;
        pickerView.hidden = !isVisible;
    ...
    else if(listType == 1)
    {
        ...
        tableView.backgroundColor = self->listBackColor; //for previous version of iOS 8
        tableView.separatorColor = self->listFontColor;
        tableView.hidden = !isVisible;
...
-(void)destroyRunObject:(BOOL)bFast;
{
    ...
    if(listBackColor != nil)
       [listBackColor release];
    if(listFontColor != nil)
       [listFontColor release];
}
...
{
    index = clamp(index+indexOffset, 0, [list size]-1);
...
-(UITableViewCell*)tableView:(UITableView*)tView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //Uncomment these two lines to make text be multiline in a single item
...
{
    [self defineColors];
    cell.backgroundColor = self->listBackColor;
    cell.textLabel.textColor = self->listFontColor;
}
...
-(void) defineColors
{
    ...
    listBackColor = [[UIColor alloc] initWithRed:(float)getR(listFontBack)/255.0f
                                           green:(float)getG(listFontBack)/255.0f
                                            blue:(float)getB(listFontBack)/255.0f
                                           alpha:1.0];
    
    int forecolor = swapRGB(listFontFore);

    if(listFontColor != nil)
       [listFontColor release];
    
    listFontColor = [[UIColor alloc] initWithRed:(float)getR(forecolor)/255.0f
                                           green:(float)getG(forecolor)/255.0f
                                            blue:(float)getB(forecolor)/255.0f
                                           alpha:1.0];

    if((flags & LIST_SYSCOLOR)!= 0)
    {
        listBackColor = [UIColor whiteColor];
        listFontColor = [UIColor blackColor];
    }
