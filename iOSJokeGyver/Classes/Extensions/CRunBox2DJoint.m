...
short jDestroyRunObject(LPRDATA rdPtr, int fast)
{
    ...
    // Build 285.6: destroy joints for objects that still exist
    VerifyJoints(rdPtr);
	int n;
...
short jHandleRunObject(LPRDATA rdPtr)
{
	if (!rStartObject(rdPtr) || rdPtr->base->bPaused)
		return 0;
...
void VerifyJoints(LPRDATA rdPtr)
{
	...
	for (n = 0; n < rdPtr->joints->Size(); n++)
	{
		...
		if (!bFlag)
		{
            //rdPtr->base->pDestroyJoint(rdPtr->base, pJointO->m_joint);    // no need here as body is destroyed => joint is already destroyed
            delete pJointO;
...
CJointO::CJointO(CRunMBase* pBase1, CRunMBase* pBase2, b2RevoluteJoint* joint)
{
	...
	{
		pHo = pBase1->m_pHo;
		m_fv1 = (pHo->hoCreationId*65536)|(pHo->hoNumber&0xFFFF);
	}
	m_fv2 = -1;
	if (pBase2 != NULL)
	{
		pHo = pBase2->m_pHo;
		m_fv2 = (pHo->hoCreationId*65536)|(pHo->hoNumber&0xFFFF);
	}
	m_joint = joint;    
}
...
{
    m_object = calloc(sizeof(RUNDATA), 1);
