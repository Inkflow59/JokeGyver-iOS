    ...
    switch(action)
    {
        ...
        case ACTION_WAITFORCOMMAND:
            ...
            if (flags&SCR_SENDATSTART)
            {
                ...
                [gameCenter sendScore:scoreSet identifier:identifier];
            ...
            {
                flags&=~SCR_GETATSTART;
                [gameCenter getScores:identifier timeScope:timeScope range:range];
            ...
            {
                flags&=~SCR_GETTITLE;
                [gameCenter getTitle:identifier];
    ...
    else
    {
        ...
        if (0 != (flags & SCR_NAMEFIRST))
        {
            ...
            for (int a = 0; a < maxScores; a++)
            {
                ...
                CRect tmpRect = rc;
                CGSize scoreSize = [score sizeWithFSFont:[font createFont]];
        ...
        else
        {
            ...
            for (int a = 0; a < maxScores; a++)
            {
                ...
                CGSize nameSize = [name sizeWithFSFont:[font createFont]];
...
-(void)setIdentifier:(CActExtension*)act
{
    identifier=[[NSString alloc] initWithString:[act getParamExpString:rh withNum:0]];
}
...
- (void)gameCenterViewControllerDidFinish:(nonnull GKGameCenterViewController *)gameCenterViewController {
    ...
    [ho->hoAdRunHeader->rhApp UpdateViewport];
}
-(void)displayDefault
{
    if (gameCenter!=nil)
    {
        if (gameCenter->localPlayer!=nil)
        {
            if (gameCenter->localPlayer.isAuthenticated)
            {
                ...
                {
                    gameCenterController.leaderboardIdentifier=identifier;    //category = Identifier
                    gameCenterController.leaderboardTimeScope=(GKLeaderboardTimeScope)timeScope;
                    [ho->hoAdRunHeader pausePlay];
                    gameCenterController.gameCenterDelegate=self;
                    [ho->hoAdRunHeader->rhApp->mainViewController presentViewController:gameCenterController animated:YES];
                }
            }
        }
...
-(CValue*)getIdentifier
{
    return [rh getTempString:identifier];
}
-(CValue*)getName
{
...
-(CValue*)getHighScore
{
    ...
    if (gameCenter!=nil && gameCenter->localPlayer!=nil)
    {
        if (gameCenter->localPlayer.isAuthenticated)
        {
            [ret forceInt:[gameCenter getLBLocalPlayerHighScore]];
        }
    }
    return ret;
}
