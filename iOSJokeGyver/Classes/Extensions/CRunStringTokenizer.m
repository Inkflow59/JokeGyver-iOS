...
{
    int count = 0;
    BOOL tokenFound = false; // Set when a non-delimiter is found
    int tmpPos = pos;
    
    while (tmpPos < len)
    {
        if ([self indexOfFromCharAt:delimiter fromText:text at:tmpPos++] >= 0)
        {
            if (tokenFound)
            {
        ...
        else
        {
            ...
            while (tmpPos < len
                && [self indexOfFromCharAt:delimiter fromText:text at:tmpPos] < 0)
                ++tmpPos;
        }
    }
...
{
    if (pos < len
        && [self indexOfFromCharAt:delimiter fromText:text at:pos] >= 0)
    {
        while (++pos < len
            && [self indexOfFromCharAt:delimiter fromText:text at:pos] >= 0) {};
    }
...
-(int)indexOfFromCharAt:(NSString*)text0 fromText:(NSString*)text1 at:(int)pos
{
    NSRange range = [text0 rangeOfString:[NSString stringWithFormat: @"%C",[text1 characterAtIndex:pos]]];
