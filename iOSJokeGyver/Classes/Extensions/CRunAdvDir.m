...
-(BOOL)CompDist:(int)p1 withPos:(int)p2 andParam:(int)v
{
	int x1 = POSX(p1);
	int y1 = POSY(p1);
	int x2 = POSX(p2);
...
-(BOOL)CompDir:(int)p1 withPos:(int)p2 andParam:(int)dir andParam2:(int)offset
{
    int x1 = POSX(p1);
    int y1 = POSY(p1);
    int x2 = POSX(p2);
...
-(void)GetObjects:(CObject*)object withParam:(int)position
{
	...
	it->value=((object->hoCreationId * 65536) + object->hoNumber);
