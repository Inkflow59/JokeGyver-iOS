...
{ //occurance is 0-based
	...
	if ([s length]==0 || find == nil || [find length] == 0)
	{
		return -1;
	}
	if (!caseSensitive)
	...
	{ //no wildcards
		NSInteger lastIndex = -[find length];
		for (int i = 0; i <= occurance; i++)
		{
			range.location=lastIndex+[find length];
			range.length=[theSource length]-range.location;
...
-(CValue*)expression:(int)num
{
	switch (num)
	{
		...
		case EXP_LISTAPPEND:
            if ( [delims size] > 0 && source.length > 0 )
            {
                NSString* tmp = [(NSString*)[delims get:0] stringByAppendingString:[[ho getExpParam] getString]];
                return [rh getTempString:[source stringByAppendingString:tmp]];
            }
            else
		...
		case EXP_LISTPREPEND:
            if ( [delims size] > 0 && source.length > 0 )
            {
                NSString* tmp = [(NSString*)[delims get:0] stringByAppendingString:source];
                return [rh getTempString:[[[ho getExpParam] getString] stringByAppendingString:tmp]];
            }
            else
