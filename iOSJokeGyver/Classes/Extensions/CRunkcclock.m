...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
	...
	sVisible = ho->ros->rsFlags & RSFLAG_VISIBLE;
	startTimer = nil;
	stopTimer = nil;
	sDisplay = true;
...
{
	if(updateAnalog)
		[textSurface manualClear:ABGRtoRGB(crFont)];
	...
	if (sAnalogClockLines)
	{
		for (int a = 1; a < 13; a++)
		{
			...
			renderer->renderLine(vCenter + pA, vCenter + pB, ABGRtoRGB(crFont), 2);
		}
	}
	...
	if (updateAnalog && sAnalogClockMarkerType != 2)
	{
		...
		for (int a = 1; a <= 12; a++)
		{
			...
			CGSize textSize = [szString sizeWithFSFont:hFont->font];
			...
			[textSurface manualDrawText:szString withFlags:0 andRect:rcFont andColor:ABGRtoRGB(crFont) andFont:hFont];
		}
...
-(void)runDisplayDigitalTime:(CRenderer*)renderer andString:(NSString*)szTime andRect:(CRect)rc
{
	...
	[textSurface setText:szTime withFlags:DT_CENTER|DT_VCENTER andColor:ABGRtoRGB(crFont) andFont:hFont];
...
-(void)runDisplayCalendar:(CRenderer*)renderer andString:(NSString*)szDate andRect:(CRect)rc
{
	...
	[textSurface setText:szDate withFlags:DT_CENTER|DT_VCENTER andColor:ABGRtoRGB(crFont) andFont:hFont];
	[textSurface draw:renderer withX:rc.left andY:rc.top andEffect:0 andEffectParam:0];
}
