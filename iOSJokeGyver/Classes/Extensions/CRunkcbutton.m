...
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo*)cob andVersion:(int)version
{
    ...
    foreColour = swapRGB([file readAInt]);
    backColour = swapRGB([file readAInt]);
    text = nil;
    button = nil;
    bswitch = nil;
    ...
    switch(buttonType)
    {
        ...
        case BTNTYPE_PUSHTEXTBITMAP:
        {
            ...
            button=[UIButton buttonWithType:(UIButtonType)0];
            break;
        }
    ...
    {
        control = button;
        if(buttonType != BTNTYPE_PUSHTEXT || (flags & BTN_TRANSP_BKD)!=0)
            button.backgroundColor = [UIColor clearColor];
        else
        {
            UIColor* color = [UIColor lightGrayColor];
            if((flags & BTN_SYSCOLOR) == 0)
                color = [UIColor colorWithRed:(float)getR(backColour)/255.0f
                                        green:(float)getG(backColour)/255.0f
                                         blue:(float)getB(backColour)/255.0f alpha:1.0f];
...
-(int)handleRunObject
{
    if(control != nil && rh->rhApp->subApp != nil)
    {
