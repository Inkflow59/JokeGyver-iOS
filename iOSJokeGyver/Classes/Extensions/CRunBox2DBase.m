...
void GetObjects(LPRDATABASE rdPtr)
{
    ...
    for (nObjects = 0; nObjects<rdPtr->rh->rhNObjects; pOL++, nObjects++)
    {
        ...
        if (pObject->hoType >= 32)
        {
            ...
            {
                rdPtr->ropes->Add(((CRunBox2DParent*)pExtension->ext)->m_object);
            }
        }
    }
}
...
void rDestroyBody(void* ptr, b2Body* pBody)
{
	...
	if (pMBase != nil && pMBase->m_type!=MTYPE_PLATFORM && pMBase->m_type!=MTYPE_OBSTACLE)
	{
		...
		for (n=0; n < rdPtr->ropes->Size(); n++)
		{
			pRope = (RUNDATARC*)rdPtr->ropes->Get(n);
			pRope->pRemoveObject(pRope, pMBase);
		}
	}
    ...
    if(!rdPtr->bListener)
        rdPtr->world->SetContactListener(nil);
}
...
//				float angle=GetAnimDir(pHo, pHo->roc->rcDir)*11.25f;
				...
				rdPtr->objectIDs->AddInt((pHo->hoCreationId*65536)|(pHo->hoNumber&0xFFFF));
			}
		}
	}
}
...
// Build 286.0
// Adds a rope to an engine and returns YES if it was really added
BOOL b2brAddRope(void* ptr, void* pObject)
{
	LPRDATABASE rdPtrBase=(LPRDATABASE)ptr;
	RUNDATARC* pRope = (RUNDATARC*)(((CRunBox2DParent*)((CExtension*)pObject)->ext)->m_object);
	if ( pRope->identifier == rdPtrBase->identifier)
	{
		// Add rope to engine
		rdPtrBase->ropes->Add(pRope);
		return YES;
	}
	return NO;
}
...
short b2bHandleRunObject(LPRDATABASE rdPtr)
{
    ...
    {
        if(rdPtr->bListener)
            rdPtr->world->SetContactListener(nil); // this will set contact listener to null, then no more collision detect
        rdPtr->bListener = false;
        return 0;
    }
    else
    {
        if(!rdPtr->bListener)
            rdPtr->world->SetContactListener(rdPtr->contactListener); // restore the listener to continue detect collisions
        rdPtr->bListener = true;
    }
...
short RACTION_PAUSE(LPRDATABASE rdPtr)
{
    rdPtr->bPaused = true;
    return 0;
}
short RACTION_RESUME(LPRDATABASE rdPtr)
{
    rdPtr->bPaused = false;
    return 0;
}
...
{
	CObject* pHo=[act getParamObject:rdPtr->rh withNum:0];
	if (pHo->hoType == 2 && rdPtr->objects->IndexOf(pHo)<0)
	{
    ...
    else if (movement1->m_type == MTYPE_PLATFORM || movement2->m_type == MTYPE_PLATFORM)
    {
		...
		switch (movement->m_type)
		{
            case MTYPE_OBJECT:
                ...
                {
                    // Build 286.0: don't disable contact if the platform is under the feet of the object
                    int x, y;
                    float angle;
                    bool bPlatformUnder = false;
                    movement->m_base->pGetBodyPosition(movement->m_base, movementB->m_body, &x, &y, &angle);
                    int left = (int)(x + movementB->rc.left);
                    int right = (int)(x + movementB->rc.right);
                    int bottom = (int)(y + movementB->rc.bottom);
                    // 		int top = (int)(y + movementB->rc.top);
                    if (pHo->hoX >=left && pHo->hoX <= right && pHo->hoY <= bottom)
                        bPlatformUnder = true;
                    if ( !bPlatformUnder )
                    {
                        if (velocity.y>=0.0f)
...
{
    m_object = calloc(sizeof(RUNDATABASE), 1);
    
    LPEDATABASE object = (LPEDATABASE)calloc(sizeof(EDITDATABASE), 1);
