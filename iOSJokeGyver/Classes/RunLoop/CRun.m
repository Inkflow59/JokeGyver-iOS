...
-(int)allocRunHeader
{
	...
	rh4PosOnLoop = nil;
    rh4ComplexOnLoop = NO;
    rhChildEventToExecute = nil;
    rhChildEventSelectionStack.clear();
	...
	rhJoystickMask=0xFF;
	rh4MvtTimerCoef=0;
	rhLastCreatedObjectFixedValue = 0;
...
-(int)initRunLoop:(BOOL)bFade
{
    ...
    // Not sure why InitLoop was moved below but things it contains must be done before redrawLevel
    // Just in case, I split it in 2 parts and call the first part here
    [self f_InitLoop_part1];
...
-(int)doRunLoop
{
	...
	if ((rhGameFlags & GAMEFLAGS_FIRSTLOOPFADEIN) != 0)
	{
		...
		rhFrame->fadeTimerDelta = (int)([self getAbsoluteTime]-rhTimerOld);
	...
	if (quit != 0)
	{
		switch (quit)
		{
			...
			case -2:	    // LOOPEXIT_ENDGAME:
				[rhEvtProg handle_GlobalEvents:((-4 * 65536) | 65533)];	// CNDL_QUITAPPLICATION
				break;
		}
	}
	return quit;
...
-(int)prepareFrame
{
	...
	[rhEvtProg prepareQualifierList:rhOiList withMaxOI:rhMaxOI];
...
-(void)createRemainingFrameObjects
{
	...
	// Remet le timer
	rhTimerOld = [self getAbsoluteTime] - rhFrame->fadeTimerDelta;
...
-(void)resetFrameLayers:(int)nLayer withFlag:(BOOL)bDeleteFrame
{
	...
	for (; i < nLayers; i++)
	{
		...
		for (j = 0; j < nLOs; j++)
		{
            ...
            COI* oiPtr = [rhApp->OIList getOIFromHandle:plo->loOiHandle];
            if(oiPtr != nil && oiPtr->oiOC != nil && oiPtr->oiOC->ocEffect != nil)
            {
                [oiPtr->oiOC->ocEffect release];
                oiPtr->oiOC->ocEffect = nil;
            }
		}
		...
		if (pLayer->pBkd2 != nil)
		{
			for (j = 0; j < [pLayer->pBkd2 size]; j++)
			{
                ...
                if(pbkd->bkdEffect != nil)
                   [pbkd->bkdEffect release];
			}
			...
			if ((oiPtr->oiFlags & OIF_GLOBAL) != 0)
			{
				...
				while (YES)
				{
					...
					else
					{
						...
						saveValues->pStrings = (NSString**)malloc(hoPtr->rov->rvNumberOfStrings*sizeof(NSString*));
					...
					if ([name compare:save->name] == 0)
					{
						...
						while (true)
						{
							...
							else
							{
                                ...
                                if(hoPtr->rov->rvNumberOfStrings < saveValues->rvNumberOfStrings)
                                    GrowAlterableStrings(hoPtr->rov, saveValues->rvNumberOfStrings);
...
-(int)f_CreateObject:(short)hlo withOIHandle:(short)oi andX:(int)coordX andY:(int)coordY andDir:(int)initDir andFlags:(short)flags andLayer:(int)nLayer andNumCreation:(int)numCreation
{
	...
	{		
		...
		// Qualifier optimization: add OIL to qualifier OIL list
		CObjInfo* oilPtr = hoPtr->hoOiList;
		if (oilPtr->oilRealQualifiers != nil)
		{
			short oil = oilPtr->oilIndex;
			for(int qn = 0; oilPtr->oilRealQualifiers[qn] >= 0; qn++)
			{
				// Check if OIL in qualToOiList list for this qualifier
				BOOL bIn = NO;
				int noi;
				short qualNum = oilPtr->oilRealQualifiers[qn];
				CQualToOiList* currentOIsPerQualifier = rhEvtProg->qualToOiList[qualNum];
				int firstFreePos = currentOIsPerQualifier->nQoi;
				for(noi = 0; noi < firstFreePos; noi += 2)
				{
					if (currentOIsPerQualifier->qoiList[noi+1] < 0) // not needed I think
					{
						firstFreePos = noi;
						break;
					}
				...
				// Not already in qualToOiList list for this qualifier => add it
				if (!bIn)
				{
					CQualToOiList* allOIsPerQualifier = rhEvtProg->qualToOiListFull[qualNum];
					
					// Find OIL in allOIsPerQualifier
					int oilPos = -1;
					for (noi = 0; noi < allOIsPerQualifier->nQoi; noi += 2)
					{
					...
					if (oilPos >= 0)
					{
						...
						{
							// Find insertion point (= position in currentOIsPerQualifier of the first following OIL in allOIsPerQualifier
							for (oilPos += 2; bAddToEnd && oilPos < allOIsPerQualifier->nQoi; oilPos += 2)
							{
								short nextOil = allOIsPerQualifier->qoiList[oilPos + 1];
								for (noi = 0; noi < currentOIsPerQualifier->nQoi; noi += 2)
								{
        ...
        // Build 290: copied this from object init at start of frame
        if ((hoPtr->hoOEFlags & OEFLAG_MANUALSLEEP) == 0)
        {
            ...
            if ((hoPtr->hoLimitFlags & OILIMITFLAGS_QUICKBACK) != 0)
            {
                ...
                if ((rhFrame->leFlags & LEF_TOTALCOLMASK) != 0)
                {
                    hoPtr->hoOEFlags |= OEFLAG_NEVERSLEEP;
                }
            }
		...
		// --------------------------------------------
        if ((oiPtr->oiLoadFlags & OILF_ELTLOADED) == 0)
        {
		...
		rhLastCreatedObjectFixedValue = (hoPtr->hoCreationId*65536)|(hoPtr->hoNumber&0xFFFF);
...
-(void)f_KillObject:(int)nObject withFast:(BOOL)bFast
{
    ...
    if ( poil != nil && poil->oilObject == (short)0x8000 )        // last object instance?
    {
        //NSLog(@"Last instance of: %@", poil->oilName);

        // Is the object discardable?
        COI* oiPtr = [rhApp->OIList getOIFromHandle:poil->oilOi];
        if ( oiPtr != nil && (oiPtr->oiFlags & OIF_DISCARDABLE) != 0 )
        {
            NSLog(@"Discard: %@", poil->oilName);
			...
			for (count = 0; dw != 0 && count < 32; count++)
			{
				if ((dw & 1) != 0)
				{
					...
					if (pHo != nil)
					{
						...
						{
							[rhEvtProg handle_Event:pHo withCode:(pHo->hoType| (-33 * 65536))];	    // CNDL_EXTNOMOREOBJECT
							
							// Qualifier optimization
							// Remove OIL from qualifier OIL list
							if (pHo->hoOiList->oilRealQualifiers != nil)
							{
								short oil = pHo->hoOiList->oilIndex;
								for(int qn = 0; pHo->hoOiList->oilRealQualifiers[qn] >= 0; qn++)
								{
									short qualNum = pHo->hoOiList->oilRealQualifiers[qn];
									int noi;
									CQualToOiList* currentOIsPerQualifier = rhEvtProg->qualToOiList[qualNum];
									for(noi = 0; noi < currentOIsPerQualifier->nQoi; noi += 2)
									{
										if (currentOIsPerQualifier->qoiList[noi+1] == oil)
										{
											for(; noi < currentOIsPerQualifier->nQoi - 2; noi += 2)
											{
...
{
	if ((hoSource->hoFlags & HOF_SHOOTER) != 0)
	{
		int count = 0;
		int nObject;
		CObject* hoPtr;
...
-(void)pause
{
	...
	if (rh2PauseCompteur == 1)
	{
		...
		// Sauve le timer
		// ~~~~~~~~~~~~~~
		rh2PauseTimer = [self getAbsoluteTime];
...
-(void)resume
{
    ...
    if (rh2PauseCompteur>0)
    {
		...
		if (rh2PauseCompteur == 0)
        {
            ...
            // Remet le timer
            // --------------
            double tick = [self getAbsoluteTime];
            tick -= rh2PauseTimer;
            rhTimerOld += tick;
            ...
            [rhEvtProg handle_GlobalEvents:((-8*65536)|0xFFFD)];
        }
	}
}

/** Goes into pause mode hardware.
 */
-(void)pause2
{
    // Le compteur de sauvegarde
    ...
    if (rh2PauseCompteur == 1)
    {
        ...
        // Arret des musiques et des sons
        // ------------------------------
        [rhApp->soundPlayer pause:2];
    }
}
/** Quits the pause mode hardware.
 */
-(void)resume2
{
    // Uniquement au dernier retour
...
-(void)scrollLevel
{
	...
	if (xCoef != 1.0f)
	{
		oX = round((float) oX * xCoef);
		nX = round((float) nX * xCoef);
	}
...
{	
	...
	if (rh3Scrolling!=0)
	{
		...
		{
            int flags = DLF_DONTUPDATE|DLF_DRAWOBJECTS|DLF_REDRAWLAYER;
            [self redrawLevel:flags];
		}
	}
						...
						if ( nLayer > 0 || !bLayer0Invisible)
						{
							...
							if ( *ppSpr == nil )
							{
								switch ( typeObj ) {
                                    ...
                                    {
                                        int shader = -1;
                                        if ((poi->oiInkEffect & BOP_MASK) == BOP_EFFECTEX)
                                            shader = [poc checkOrCreateEffectIfNeeded:rhApp andCOI:poi];
										*ppSpr = [spriteGen addOwnerDrawSprite:(int)rc.left withY1:(int)rc.top andX2:(int)rc.right andY2:(int)rc.bottom andLayer:plo->loLayer andZOrder:i*4+nCurSprite andBackColor:0 andFlags:(dwFlags | SF_COLBOX) andObject:nil andDrawable:poc];
                                        [spriteGen modifSpriteEffect:*ppSpr withInkEffect:poi->oiInkEffect andInkEffectParam:poi->oiInkEffectParam andEffectShader:shader];
										break;
                                    }
										// Backdrop: sprite
									case OBJ_BKD:
	...
	for (int nObjects = 0; nObjects < rhNObjects; nObjects++)
	{
		...
		if (noMove != 0)
		{
			if ((pHo->hoOEFlags & OEFLAG_SCROLLINGINDEPENDANT) != 0)
			{
                ...
                //NSLog(@"layer: %d, with x:%d and y: %d and delta x:%d, y:%d",pHo->hoLayer, x, y, rh4WindowDeltaX, rh4WindowDeltaY);
				if (pHo->rom == nil)
				{
					pHo->hoX += x;
					pHo->hoY += y;
...
{
	CBkd2* toadd;
	toadd = [[[CBkd2 alloc] initWithCRun:self] retain];
    ...
    if(hoPtr->ros->rsEffectShader != -1)
    {
        COI* poi = [rhApp->OIList getOIFromHandle:hoPtr->hoOi];
        toadd->bkdEffectShader = [toadd checkOrCreateEffectIfNeeded:rhApp andCOI:poi];
    }
...
-(void)addBackdrop2:(CBkd2*)toadd
{
			...
			if (pbkd->x == toadd->x && pbkd->y == toadd->y && pbkd->nLayer == toadd->nLayer && pbkd->img == toadd->img && (pbkd->inkEffect & EFFECT_MASK) == EFFECT_NONE)
			{
				...
				if (pbkd->inkEffect != toadd->inkEffect || pbkd->inkEffectParam != toadd->inkEffectParam)
				{
					...
					for (int j = 0; j < 4; j++)
					{
						if (pbkd->pSpr[j] != nil)
						{
                            [spriteGen modifSpriteEffect:pbkd->pSpr[j] withInkEffect:pbkd->inkEffect andInkEffectParam:pbkd->inkEffectParam andEffectShader:pbkd->bkdEffectShader];
						}
					}
				}
				[toadd release];
	...
	int nPlayfieldWidth = rhFrame->leWidth;
	int nPlayfieldHeight = rhFrame->leHeight;
	BOOL bLayerVisible = ((pLayer->dwOptions & (FLOPT_TOHIDE | FLOPT_VISIBLE)) == FLOPT_VISIBLE);

	{
		...
		do
		{
			...
			if (bTotalColMask || (rc.left < x2edit + COLMASK_XMARGIN + 32 && rc.top < y2edit + COLMASK_YMARGIN))
			{
				...
				// Display object?
				if (bLayerVisible && CRectIntersects(rc, pLayer->visibleRect))
				{
					////////////////////////////////////////
					// Non-background layer => create sprite
					...
					[spriteGen modifSpriteEffect:pbkd->pSpr[nCurSprite] withInkEffect:pbkd->inkEffect andInkEffectParam:pbkd->inkEffectParam andEffectShader:pbkd->bkdEffectShader];
				}
...
-(void)f_InitLoop_part2
{
	...
	rhTimerOld = [self getAbsoluteTime];
	rhTimer = 0;
	...
	if (rhApp->parentApp==nil)
	{
		...
		else 
		{
            ...
            if ((rhFrame->iPhoneOptions&IPHONEOPT_JOYSTICK_DPAD)!=0)
            {
                flags|=JFLAG_DPAD;
            }
...
{ 
	...
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	double timerBase = [self getAbsoluteTime];
	double delta = timerBase - rhTimerOld;
	double oldtimer = rhTimer;
	rhTimer = delta;
	delta -= oldtimer;
	rhTimerDelta = round(delta);				// Delta a la boucle precedente
	...
	if (b != 0)
	{
		...
		if ((b & 0xF0) != 0)
		{
			...
			if ((b & 0xF0) != 0)
			{
				...
				[rhEvtProg handle_GlobalEvents:((-4 * 65536) | 0xFFF9)];	// CNDL_JOYPRESSED);
			}
			// Les autres touches...
			if ((b & 0x0F) != 0)
			{
		...
		rhQuit == LOOPEXIT_NEWGAME)
	{
		...
		[rhEvtProg handle_GlobalEvents:(-2 * 65536) | 0xFFFD];
	}
    
    //NSLog(@"Mouse X: %d, Y: %d", self->rh2MouseX, self->rh2MouseY);
	return rhQuit;
}
...
-(void)getMouseCoords
{
    ...
    rh2MouseX = rhApp->mouseX;
    rh2MouseY = rhApp->mouseY;
    //NSLog(@"Mouse Coords, x:%d, y:%d, Window: %d, %d and Parent: %d %d", rh2MouseX, rh2MouseY, rhWindowX, rhWindowY, rhApp->parentX, rhApp->parentY);
...
-(BOOL)newHandle_Collisions:(CObject*)pHo
{
	...
	if ((pHo->hoLimitFlags & OILIMITFLAGS_QUICKBORDER) != 0)
	{
		...
		if (cadran1 != 0)		// Si deja dedans, on ne teste pas
		{
			...
			if (cadran2 == 0)		// Teste si entre!
			{
				...
				if (chgDir != 0)
				{
					...
					int saveParam = rhEvtProg->rhCurParam[0];
					rhEvtProg->rhCurParam[0] = chgDir;
					[rhEvtProg handle_Event:pHo withCode:(-11 * 65536) | (((int) pHo->hoType) & 0xFFFF)];  // CNDL_EXTINPLAYFIELD
					rhEvtProg->rhCurParam[0] = saveParam;
				}
			}
		}
		...
		{
			BOOL oldMoveFlag = pHo->rom->rmMoveFlag;
			...
			if (pHo->roc->rcMovementType != MVTYPE_PLATFORM && pHo->roc->rcMovementType != MVTYPE_EXT)
		...
		if (cadran1 != BORDER_ALL)		// Si deja completement dehors, on ne teste pas
		{
			...
			if (chgDir != 0)
			{
				...
				[rhEvtProg handle_Event:pHo withCode:(-12 * 65536) | (((int) pHo->hoType) & 0xFFFF)];  // CNDL_EXTOUTPLAYFIELD
				rhEvtProg->rhCurParam[0] = saveParam;
			}
		 }
	 }
		 ...
		 if (cnt != nil)
		 {
			 ...
			 for (obj = 0; obj < [cnt size]; obj++)
			 {
				 ...
				 if ((pHox->hoFlags & HOF_DESTROYED) == 0)	// Detruit au cycle precedent?
				 {
					 ...
					 [rhEvtProg handle_Event:pHo_esi withCode:(-14 * 65536) | (((int) type) & 0xFFFF)];	// CNDL_EXTCOLLISION
					 rhEvtProg->rhCurParam[0] = saveParam;
				 }
			 }
			 [cnt release];
...
-(int)colMask_TestObject_IXY:(CObject*)pHo withImage:(short)newImg andAngle:(float)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andX:(int)newX andY:(int)newY andFoot:(int)htfoot andPlane:(int)plan
{
	...
	if (bSprite)
	{
		...
		if (pSpr != nil)
		{
			...
			{
				res = ((-13 * 65536) | (((int) pHo->hoType) & 0xFFFF));	    // CNDL_EXTCOLBACK
			}
		}
	}
	else
...
-(void)actPla_FinishLives:(int)joueur withLive:(int)live
{
	...
	if (live == 0)
	{
		if (lives[joueur] != 0)
		{
			[rhEvtProg push_Event:0 withCode:((-5 * 65536) | 0xFFF9) andParam:0 andObject:nil andOI:(short)joueur];
		}
	}
...
+(CFontInfo*)getObjectFont:(CObject*)hoPtr
{
	...
	CFontInfo* info2=[[[CFontInfo alloc] init] autorelease];
	[info2 copy:info];
	return info2;
}
...
-(void)callEventExtension:(CExtension*)hoPtr withCode:(int)code andParam:(int)param
{
	if (rh2PauseCompteur == 0)
	{
        ...
        code = (-(code + EVENTS_EXTBASE + 1) * 65536);
    ...
    for (nObjects=0; nObjects<rhNObjects; pOL++, nObjects++)
    {
        ...
        if (pHo->rom != nil)
        {
            ...
            if (pHo->roc->rcMovementType==MVTYPE_EXT)
            {
				...
				if ( (mvPtr->mvRuntimeFlags & MVRTFLAG_PHYSICS) != 0 )
                {
