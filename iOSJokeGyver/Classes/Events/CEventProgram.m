...
-(id)initWithApp:(CRunApp*)a
{
    ...
    gEvgFlagsDefaultMask=EVGFLAGS_DEFAULTMASK;
	qualToOiList = nil;
	qualToOiListFull = nil;
	onEachLoopConditionIndexes = nil;
	onEachLoopConditionIndexesActives = nil;
	return self;
}
-(void)dealloc
{
...
-(void)load
{
	...
	if (onEachLoopConditionIndexes != nil)
	{
		[onEachLoopConditionIndexes release];
		onEachLoopConditionIndexes = nil;
	}
	if (onEachLoopConditionIndexesActives != nil)
	{
		[onEachLoopConditionIndexesActives release];
		onEachLoopConditionIndexesActives = nil;
	}
	while (true)
	{
		...
		if (code[0] == 'E' && code[1] == 'R' && code[2] == '>' && code[3] == '>')
		{
			...
			if (nConditions[NUMBEROF_SYSTEMTYPES + OBJ_SPR] < CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS+1)
				nConditions[NUMBEROF_SYSTEMTYPES + OBJ_SPR] = CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS+1;
        ...
        {
            int options = [app->file readAInt];
            if ( (options & EVENTOPTION_BREAKCHILD) != 0 )
                gEvgFlagsDefaultMask |= EVGFLAGS_BREAK;
        }
		// EVTFILECHUNK_END
...
-(LPEVG)inactiveGroup:(LPEVG)evgPtr
{
	...
	evgPtr->evgFlags &= gEvgFlagsDefaultMask;
...
-(void)prepareProgram
{
	...
	for (evgPtr = pEvents; evgPtr->evgSize!=0;)
	{
		...
		evtPtr = EVGFIRSTEVT(evgPtr);
		if (evtPtr->evtCode.evtLCode.evtCode == ((-10 * 65536) | 65535))	// CNDL_GROUP)
		{
			grpPtr = (LPGRP)&EVTPARAMS(evtPtr)->evp.evpW.evpW0;
...
// Prepare list of OIs for each typed qualifier
-(void)prepareQualifierList:(CObjInfo**)oiList withMaxOI:(int)maxOI
{
	...
	if (nQualifiers > 0)
	{
		...
		for (q = 0; q < nQualifiers; q++)
		{
			...
			for (oil = 0; oil < maxOI; oil++)
			{
				if (oiList[oil]->oilType == qualifiers[q]->qType)
				{
					for (n = 0; n < 8 && oiList[oil]->oilQualifiers[n] != -1; n++)      // MAX_QUALIFIERS
					{
						if (oi == oiList[oil]->oilQualifiers[n])
						{
							count[q]++;
							oiList[oil]->oilEventCount++;
						}
					}
				}
			}
		...
		qualToOiListFull = (CQualToOiList**)malloc(nQualifiers*sizeof(CQualToOiList*));
		for (q = 0; q < nQualifiers; q++)
		{
			qualToOiList[q] = [[CQualToOiList alloc] init];
			qualToOiListFull[q] = [[CQualToOiList alloc] init];

			if (count[q] != 0)
			{
				qualToOiList[q]->qoiList = (short*)malloc( (count[q]*2+2)*sizeof(short) );
				qualToOiList[q]->nQoi=0;
				qualToOiListFull[q]->qoiList = (short*)malloc( (count[q]*2+2)*sizeof(short) );
				qualToOiListFull[q]->nQoi=count[q]*2;
			}
			...
			for (oil = 0; oil < maxOI; oil++)
			{
				if (oiList[oil]->oilType == qualifiers[q]->qType)
				{
					for (n = 0; n < 8 && oiList[oil]->oilQualifiers[n] != -1; n++)
					{
						if (oi == oiList[oil]->oilQualifiers[n])
						{
							qualToOiListFull[q]->qoiList[i * 2] = oiList[oil]->oilOi;
							qualToOiListFull[q]->qoiList[i * 2 + 1] = (short) oil;
							qualToOiList[q]->qoiList[i * 2] = -1;
							qualToOiList[q]->qoiList[i * 2 + 1] = -1;
							if (oiList[oil]->oilRealQualifiers == nil)
							{
								if (oiList[oil]->oilEventCount != 0)
								{
									oiList[oil]->oilRealQualifiers = (short*)malloc((oiList[oil]->oilEventCount+1) * sizeof(short));
									oiList[oil]->oilRealQualifiers[oiList[oil]->oilEventCount] = -1;
									oiList[oil]->oilEventCount = 0;
								}
							}
							if (oiList[oil]->oilRealQualifiers != nil)
								oiList[oil]->oilRealQualifiers[oiList[oil]->oilEventCount++] = (short)q;
			...
			if (i!=0)
			{
				qualToOiListFull[q]->qoiList[i * 2] = -1;
				qualToOiListFull[q]->qoiList[i * 2 + 1] = -1;
				qualToOiList[q]->qoiList[i * 2] = -1;
				qualToOiList[q]->qoiList[i * 2 + 1] = -1;
			}
			qualToOiList[q]->qoiActionCount = -1;
			qualToOiListFull[q]->qoiActionCount = -1;
		}
...
-(short)assemblePrograms
{
	...
	short			nOi, i, j, n, num, d, evgF, evgM, d1, d2;
	int				code;
	WORD			fWrap;
	...
	do
	{
				...
				if ( (j=evtPtr->evtNParams)>0 )
				{
					for (evpPtr=EVTPARAMS(evtPtr); j>0; j--, evpPtr=EVPNEXT(evpPtr))
					{
						switch(evpPtr->evpCode)
						{
                            ...
                            case PARAM_CHILDEVENT:
                            {
                                childEventParam* pParam = (childEventParam*)&evpPtr->evp.evpW.evpW0;
                                short* pOI = pParam->objList;
                                while(*pOI != -1)
                                {
                                    *(pOI+1) = [self get_OiListOffset:*pOI withType:OBJ_SPR];
                                    pOI += 2;
                                }
                            }
                                break;
				...
				if (type>=OBJ_SPR)
				{
					switch ((int)GetEventCode(code))
					{
                        ...
                        case CNDL_EXTCOLLISION:
							...
							if (rhPtr->rhApp->hdr2Options2 & AH2OPT2_WINDOWSLIKECOLLISIONS)
								[self replaceBoxCollisionModes:evtPtr->evtOiList withOiNum:evtPtr->evtOi andOiList2:evpPtr->evp.evpW.evpW0 andOiNum2:evpPtr->evp.evpW.evpW1];
                            n=3;
                            break;
		...
		*colList=-1;
		...
		bool bOptimizeForEach = false;

		// Already optimized?
		if (onEachLoopConditionIndexes == nil && onEachLoopConditionIndexesActives == nil)
		{
			// No, find all On Each loop names, and test if all On Each conditions use constant names and there is no For Each of Two action
			onEachLoopConditionIndexes = [[NSMutableDictionary alloc] init];
			onEachLoopConditionIndexesActives = [[NSMutableDictionary alloc] init];
			bOptimizeForEach = true;

			for (evgPtr=pEvents; evgPtr->evgSize!=0 && bOptimizeForEach; evgPtr=EVGNEXT(evgPtr))
			{
				for (evtPtr=EVGFIRSTEVT(evgPtr), n = 0; n < evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
				{
					type = EVTTYPE(evtPtr->evtCode.evtLCode.evtCode);
					if (type >= OBJ_SPR && GetEventCode(evtPtr->evtCode.evtLCode.evtCode) == (CNDL_EXTONLOOP & 0xFFFF0000))
					{
						// Name = simple string?
						...
						if ( pToken->expCode.expSCode.expType == -1 && pToken->expCode.expSCode.expNum == 3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode == 0 )
						{
							NSString* loopname = (NSString*)[allocatedStrings get:pToken->expu.expw.expWParam0];
							loopname = [loopname uppercaseString];
							NSMutableDictionary* pOnEachLoopConditionIndexes = (type == OBJ_SPR) ? onEachLoopConditionIndexesActives : onEachLoopConditionIndexes;
							NSNumber* value = [pOnEachLoopConditionIndexes objectForKey:loopname];
							if (value != nil)
							{
								// Condition name already used => increment use count for this name
								int v = [value intValue] + 1;
								[pOnEachLoopConditionIndexes setValue:[NSNumber numberWithInt:v] forKey:loopname];
							}
							else
							{
								// New name => start counting
								[pOnEachLoopConditionIndexes setValue:@1 forKey:loopname];
							}
						}
						else
						{
				...
				for (n = 0; n < evgPtr->evgNAct && bOptimizeForEach; evtPtr = EVTNEXT(evtPtr), n++)
				{
					type = EVTTYPE(evtPtr->evtCode.evtLCode.evtCode);
					if (type >= OBJ_SPR && GetEventCode(evtPtr->evtCode.evtLCode.evtCode) == ACTL_EXTFOREACH2)
					{
						bOptimizeForEach = false;
						[onEachLoopConditionIndexes release];
						onEachLoopConditionIndexes = nil;
			...
			if (onEachLoopConditionIndexes != nil && [onEachLoopConditionIndexes count] != 0)
			{
				// If the number of loop names exceeds the maximum available condition numbers, remove the ones with the lower use count
				while ([onEachLoopConditionIndexes count] > CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER)
				{
					int min = 1000000000;
					NSString* minName = nil;
					for (NSString* key in onEachLoopConditionIndexes)
					{
						NSNumber* value = [onEachLoopConditionIndexes objectForKey:key];
					...
					if (minName != nil)
						[onEachLoopConditionIndexes removeObjectForKey:minName];
				}
		
				// Transform counts to condition numbers in OnEachLoopConditionIndexes maps
				NSArray* keys = [onEachLoopConditionIndexes allKeys];
				int cndIndex = CNDIDX_EXTONLOOP_INTERNAL_FIRST;
				for (NSString* key in keys)
				{
					[onEachLoopConditionIndexes setValue:[NSNumber numberWithInt:cndIndex++] forKey:key];
				}
			}
			// Same thing for active object loops
			if (onEachLoopConditionIndexesActives != nil && [onEachLoopConditionIndexesActives count] != 0)
			{
				while ([onEachLoopConditionIndexesActives count] > CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER_ONLYACTIVEOBJECTS)
				{
					int min = 1000000000;
					NSString* minName = nil;
					for (NSString* key in onEachLoopConditionIndexesActives)
					{
						NSNumber* value = [onEachLoopConditionIndexesActives objectForKey:key];
						if ([value intValue] < min)
						{
							minName = key;
							min = [value intValue];
					...
					if (minName != nil)
						[onEachLoopConditionIndexesActives removeObjectForKey:minName];
				}

				// Transform counts to condition numbers in OnEachLoopConditionIndexes maps
				NSArray* keys = [onEachLoopConditionIndexesActives allKeys];
				int cndIndex = CNDIDX_EXTONLOOP_INTERNAL_FIRST;
				for (NSString* key in keys)
				{
					[onEachLoopConditionIndexesActives setValue:[NSNumber numberWithInt:cndIndex++] forKey:key];
					if (cndIndex == EVENTS_EXTBASE + 1)
					{
						// Skip first conditions of active object
						cndIndex += NUMBER_OF_ACTIVEOBJECT_CONDITIONS;
					}
				}
		
				// Update number of conditions for active objects
				if (cndIndex > EVENTS_EXTBASE + 1 + NUMBER_OF_ACTIVEOBJECT_CONDITIONS)
					nConditions[NUMBEROF_SYSTEMTYPES + OBJ_SPR] += cndIndex - (EVENTS_EXTBASE + 1 + NUMBER_OF_ACTIVEOBJECT_CONDITIONS);
			}
		}
		...
		for ( evgPtr=pEvents; evgPtr->evgSize!=0; evgPtr=EVGNEXT(evgPtr) )
		{
			...
			for (evtPtr=EVGFIRSTEVT(evgPtr), n=0; n<evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
			{
				...
				// On Each condition?
				if (type >= OBJ_SPR && GetEventCode(code) == CNDL_EXTONLOOP)
				{
					if (bOptimizeForEach)
					{
						// Name = simple string?
						...
						if ( pToken->expCode.expSCode.expType == -1 && pToken->expCode.expSCode.expNum == 3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode == 0 )
						{
							...
							if (value != nil)
							{
								// Condition name in map => get condition index
								num = [value intValue];
								evtPtr->evtCode.evtSCode.evtNum = -num;
								code = evtPtr->evtCode.evtLCode.evtCode;
							}
						}
					}
				...
				if (bOrBefore && evtPtr->evtCode.evtLCode.evtCode != CNDL_ENDCHILDEVENT)
                {
					// Dans la liste des evenements ALWAYS
					if ( (evtPtr->evtFlags&EVFLAGS_ALWAYS)!=0 )
					{
						evtAlways++;
                        if ( (evgPtr->evgFlags & EVGFLAGS_HASPARENT) == 0 )
                            evtAlwaysRoot++;
                    }
                ...
                if ( evtPtr->evtCode.evtLCode.evtCode == CNDL_ENDCHILDEVENT )
                {
                    evtAlways++;                // for ending evgOffset (-1)
                    evgPtr->evgFlags |= EVGFLAGS_INACTIVE;        // set line as inactive
                }
            }
		}
		...
		evtAlwaysPos=0;
        std::vector< std::vector<eventOffsets> > childEventOffsetStack;
        std::vector<childEventParam*> childEventActionParamStack;
        int evtAlwaysChildrenPos = (evtAlwaysRoot+1) * sizeof(eventOffsets);
			...
			for (evtPtr=EVGFIRSTEVT(evgPtr), n=0; n<evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
			{
                ...
                if (bOrBefore && evtPtr->evtCode.evtLCode.evtCode != CNDL_ENDCHILDEVENT)
                {
					...
					if ( (evtPtr->evtFlags&EVFLAGS_ALWAYS)!=0 )
					{
                        if ( (evgPtr->evgFlags & EVGFLAGS_HASPARENT) != 0 )
                        {
                            if ( childEventOffsetStack.size() > 0 )
                            {
                                std::vector<eventOffsets>& curOffsets = childEventOffsetStack.at(childEventOffsetStack.size()-1);
                                eventOffsets eo;
                                eo.evgOffset = EVGDELTA(evgPtr, pEvents);
                                eo.evtOffset = EVTDELTA(evtPtr, pEvents);
                                curOffsets.push_back(eo);
                            }
                        }
                        else
                        {
						...
						*lposPtr+=8;
                        ...
                        if (evtPtr->evtCode.evtLCode.evtCode == CNDL_ONLOOP)
						{
                            ...
                            if ( pToken->expCode.expSCode.expType == -1 && pToken->expCode.expSCode.expNum == 3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode == 0 )
							{
								NSString* pName = (NSString*)[allocatedStrings get:pToken->expu.expw.expWParam0];
								int nOnLoop;
								CPosOnLoop* posOnLoop = nil;
								int fastLoopIndex = -1;
								[rhPtr addFastLoop:pName withIndexPtr:&fastLoopIndex];
								int nStartLoop;
								for (nStartLoop = 0; nStartLoop < posStartLoop.Size(); nStartLoop++)
								{
									CPosStartLoop* pPosStartLoop = (CPosStartLoop*)posStartLoop.Get(nStartLoop);
									if ([pPosStartLoop->m_name caseInsensitiveCompare:pName] == 0)
									{
										CCArrayList* rh4PosOnLoop = (CCArrayList*)rhPtr->rh4PosOnLoop;
										if ( rh4PosOnLoop == nil)
										...
										if (posOnLoop == nil)
										{
											for (nOnLoop = 0; nOnLoop < rh4PosOnLoop->Size(); nOnLoop++)
											{
												posOnLoop = (CPosOnLoop*)rh4PosOnLoop->Get(nOnLoop);
												if ([pName caseInsensitiveCompare:posOnLoop->m_name] == 0)
													break;
											}
											if (nOnLoop == rh4PosOnLoop->Size())
											{
										...
										pPosStartLoop->m_pEvp->evp.evpW.evpW0 = nOnLoop + 1;
									}
								}
								if (posOnLoop == nil)
								{
                ...
                // End of child event condition?
                if ( evtPtr->evtCode.evtLCode.evtCode == CNDL_ENDCHILDEVENT )
                {
                    // Copy events
                    if ( childEventOffsetStack.size() > 0 )
                    {
                        childEventParam* param = childEventActionParamStack.at(childEventActionParamStack.size()-1);
                        param->evgOffsetList = evtAlwaysChildrenPos;
                        childEventActionParamStack.pop_back();
                        
                        std::vector<eventOffsets>& curOffsets = childEventOffsetStack.at(childEventOffsetStack.size()-1);
                        eventOffsets eo;
                        eo.evgOffset = (DWORD)-1;
                        eo.evtOffset = (DWORD)-1;
                        curOffsets.push_back(eo);
                        
                        eventOffsets* eoPtr = (eventOffsets*)((LPBYTE)eventPointers+evtAlwaysChildrenPos);
                        memcpy(eoPtr, curOffsets.data(), curOffsets.size() * sizeof(eventOffsets));
                        evtAlwaysChildrenPos += curOffsets.size() * sizeof(eventOffsets);
                        childEventOffsetStack.pop_back();
                    }
                }
            }
            
            // Parent event?
            if ( (evgPtr->evgFlags & EVGFLAGS_HASCHILDREN) != 0 )
            {
                // Add new eventGroup offset list
                std::vector<eventOffsets> eos;
                childEventOffsetStack.push_back(eos);
                
                // Look for action "Start child events"
                for (n=0; n<evgPtr->evgNAct; evtPtr=EVTNEXT(evtPtr), n++)
                {
                    if ( evtPtr->evtCode.evtLCode.evtCode == ACTL_EXECUTECHILDEVENTS )
                    {
                        // Store address of ACTL_EXECUTECHILDEVENTS parameter
                        LPEVP evpPtr = EVTPARAMS(evtPtr);
                        childEventParam* param = (childEventParam*)&evpPtr->evp.evpW.evpW0;
                        childEventActionParamStack.push_back(param);
                        break;
                    }
                }
            }
...
-(void)unBranchPrograms
{
	...
	if (qualToOiListFull!=nil)
	{
		free(qualToOiListFull);
		qualToOiListFull=nil;
	}
	if (limitBuffer!=nil)
	{
...
-(void)addColList:(short)oiList withOiNum:(short)oiNum andOiList2:(short)oiList2 andOiNum2:(short)oiNum2
{
	...
	if ( oiNum < 0 )
	{
		if ( qualToOiListFull != nil )
		{
			CQualToOiList* qoil=qualToOiListFull[oiList&0x7FFF];
	...
	if ( oiNum2 < 0 )
	{
		if ( qualToOiListFull != nil )
		{
			CQualToOiList* qoil=qualToOiListFull[oiList2&0x7FFF];
            for (n=0, pOinOil = qoil->qoiList; n<qoil->nQoi; n+=2, pOinOil+=2)
			{
...
-(void)replaceBoxCollisionModes:(short)oiList withOiNum:(short)oiNum andOiList2:(short)oiList2 andOiNum2:(short)oiNum2
{
	...
	if ( oiNum < 0 )
	{
		if ( qualToOiListFull != nil )
		{
            ...
            for (n=0, pOinOil = qoil->qoiList; n<qoil->nQoi; n+=2, pOinOil+=2)
			{
				[self replaceBoxCollisionModes:*(pOinOil+1) withOiNum:*pOinOil andOiList2:oiList2 andOiNum2:oiNum2];
			}
		}
		return;
	}
	...
	if ( oiNum2 < 0 )
	{
		if ( qualToOiListFull != nil )
		{
            ...
            for (n=0, pOinOil = qoil->qoiList; n<qoil->nQoi; n+=2, pOinOil+=2)
			{
				[self replaceBoxCollisionModes:oiList withOiNum:oiNum andOiList2:*(pOinOil+1) andOiNum2:*pOinOil];
			}
		}
		return;
	}
	...
	// Normal objects
	CObjInfo* oilPtr = rhPtr->rhOiList[oiList];
	CObjInfo* oilPtr2 = rhPtr->rhOiList[oiList2];
	
	if (oilPtr->oilType == OBJ_SPR && oilPtr2->oilType == OBJ_SPR)
	{
		if ((oilPtr->oilOCFlags2 & OCFLAGS2_COLBOX) != (oilPtr2->oilOCFlags2 & OCFLAGS2_COLBOX))
		{
			short oi = oiNum;
			if ((oilPtr2->oilOCFlags2 & OCFLAGS2_COLBOX) != 0)
			{
				oilPtr = oilPtr2;
				oi = oiNum2;
			}
			COI* oiPtr = [rhPtr->rhApp->OIList getOIFromHandle:oi];
			CObjectCommon* ocPtr = (CObjectCommon*)oiPtr->oiOC;
			BOOL isPhysics = NO;
			if ((oilPtr->oilOEFlags & OEFLAG_MOVEMENTS) != 0)
			{
				if (ocPtr->ocMovements != nil && ocPtr->ocMovements->nMovements > 0)
				{
					CMoveDef* pMvt = ocPtr->ocMovements->moveList[0];
					if (pMvt->mvType == MVTYPE_EXT)
						isPhysics = ((pMvt->mvRuntimeFlags & MVRTFLAG_PHYSICS) != 0);
				}
			}
			if (!isPhysics)
			{
				oilPtr->oilOCFlags2 &= ~OCFLAGS2_COLBOX;
				
				short i = oilPtr->oilObject;
				while (i >= 0)
				{
					CObject* pHo = rhPtr->rhObjectList[i];
					if (pHo == nil)
						break;
					if (pHo->roc->rcSprite != nil)
						pHo->roc->rcSprite->sprFlags &= ~SF_COLBOX;
					i = pHo->hoNumNext;
				}
				
				if (rhPtr->rhApp->OIList->oiLoaded != nil && rhPtr->rhApp->OIList->oiLoaded[oi] != 0)
					[oiPtr enumElements:self withFont:nil];
			}
		}
	}
}
...
{
	CImage* img = [rhPtr->rhApp->imageBank getImageFromHandle:num];
	if (img != nil)
		[img setOpaqueMask:YES];
	return -1;
}
...
-(WORD)qual_GetNextOiList
{
	...
	if (qualOilPos >= qualToOiListFull[qualOilPtr]->nQoi)
	{
		return -1;
	}
	o = qualToOiListFull[qualOilPtr]->qoiList[qualOilPos + 1];
	qualOilPos += 2;
	return (o);
}
...
-(WORD)qual_GetNextOiList2
{
	...
	if (qualOilPos2 >= qualToOiListFull[qualOilPtr2]->nQoi)
	{
		return -1;
	}
	o = qualToOiListFull[qualOilPtr2]->qoiList[qualOilPos2 + 1];
	qualOilPos2 += 2;
	return (o);
}
...
-(void)SelectObjects:(short)commonOil withHoList:(CArrayList*)hoList
{
	...
	if (commonOil < 0)
		[self evt_DeselectObject:commonOil];
	...
	{
		CObject* pHo = (CObject*)[hoList get:index];
		CObjInfo* poil = pHo->hoOiList;
		short oil = poil->oilIndex;
		if (oil != oilLast)
		{
			oilLast = oil;
			poil->oilEventCount = rh2EventCount;
		...
		if ( (evgPtr->evgFlags & EVGFLAGS_INACTIVE) == 0 )	// Un groupe inhibe?
		{
			...
			if ((rhEventGroup->evgFlags&EVGFLAGS_ORINGROUP)==0)
			{
				...
				else
				{
                    ...
                    if ( (evgPtr->evgFlags & EVGFLAGS_BREAK) != 0 )
                        break;
				}
			evNextGroup:;
				pdw+=2;
					...
					if (pHo==nil)
					{
						...
						if (bTrue)
						{
							...
							while(evtPtr->evtCode.evtSCode.evtNum<0)				// Trouve les actions
								evtPtr=EVTNEXT(evtPtr);
							BOOL bBreak = ((rhEventGroup->evgFlags & EVGFLAGS_BREAK) != 0);
...
// Note: initial list may be not empty => replace OIs that already are in this list, or add new ones
-(void)evt_SaveSelectedObjects:(short*)oiOilList withObjects:(std::vector<saveSelection>&)selectedObjects
{
    short oil;
    while ((oil = *(oiOilList+1)) != -1)
    {
        CObjInfo* poil = rhPtr->rhOiList[oil];
        ...
        if ( poil->oilEventCount == rh2EventCount )
        {
            ...
            while(num>=0)
            {
                LPHO pHoFound = rhPtr->rhObjectList[num];
                if (pHoFound == NULL)
                    break;
                if ( (pHoFound->hoFlags&HOF_DESTROYED)==0 )
...
// Restore objects selected in a nested event and update their event count
-(void)evt_RestoreSelectedObjects:(short*)oiOilList withObjects:(const std::vector<saveSelection>&)selectedObjects
{
    ...
    while ((oil = *(oiOilList+1)) != -1)
    {
        ...
        {
            const saveSelection& sel = selectedObjects[i];
            if ( sel.poil == poil )
            {
                poil->oilEventCount = rh2EventCount;    // SetObjectEventCount(poil);
                poil->oilListSelected = -1;
                poil->oilNumOfSelected = 0;
                if ( sel.selectedObjects.size() > 0 )
                {
                    LPHO pHoPrev = rhPtr->rhObjectList[sel.selectedObjects[0]];
                    if ( pHoPrev != NULL )
                    {
                        poil->oilListSelected = sel.selectedObjects[0];
                        poil->oilNumOfSelected++;
                        for (int j=1; j<sel.selectedObjects.size(); j++)
                        {
...
-(void)executeChildEvents:(LPEVT)pe
{
    ...
    int newIdx = (int)rhPtr->rhChildEventSelectionStack.size();
    rhPtr->rhChildEventSelectionStack.resize(newIdx+1);
    std::vector<saveSelection>& selectedObjects = rhPtr->rhChildEventSelectionStack.at(newIdx);
    if ( newIdx > 0 )
        selectedObjects = rhPtr->rhChildEventSelectionStack.at(newIdx-1);
    [self evt_SaveSelectedObjects:pParam->objList withObjects:selectedObjects];
    
    // Execute child events (only "ALWAYS" events)
    [self computeEventList:(LPDWORD)((LPBYTE)eventPointers + pParam->evgOffsetList) withObject:(CObject*)nil];
    
    // Remove object selection from stack
    rhPtr->rhChildEventSelectionStack.pop_back();
}
...
-(void)call_Actions:(LPEVT)pActions
{
    ...
    rhPtr->rhChildEventToExecute = nil;
	...
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~
    event* childEventToExecute = rhPtr->rhChildEventToExecute;
	if ( (pEvg->evgFlags & EVGFLAGS_BREAK) != 0 && (pEvg->evgFlags & EVGFLAGS_HASPARENT) != 0 )		// Don't execute child events if the direct actions of the parent event contains a break action
		childEventToExecute = nil;
    rhPtr->rhChildEventToExecute = nil;
    ...
    if ( childEventToExecute )
    {
        [self executeChildEvents:childEventToExecute];
    }
...
-(void)onMouseButton:(int)nClicks
{
	...
	[self handle_GlobalEvents:((-5 * 65536) | 0xFFFA)];		// CNDL_MCLICK Evenement click normal
	[self handle_GlobalEvents:((-6 * 65536) | 0xFFFA)];		// CNDL_MCLICKINZONE Evenement click sur une zone
	...
	for (count = 0; count < [list size]; count++)
	{
		...
		if ((pHo->hoFlags & HOF_DESTROYED) == 0)
		{
			...
			rh4_2ndObject = pHo;
			[self handle_GlobalEvents:((-7 * 65536) | 0xFFFA)];		// CNDL_MCLICKONOBJECT
		}
	}
	[list release];
    ...
    for (int nObjects = 0; nObjects < rhPtr->rhNObjects; nObjects++)
    {
        ...
        if ((pHo->hoFlags & (HOF_REALSPRITE | HOF_OWNERDRAW)) == 0)
        {
            ...
            if (x <= mx && (x + pHo->hoImgWidth > mx))
            {
                int y = pHo->hoY - pHo->hoImgYSpot;
                if (y <= mx && (y + pHo->hoImgHeight > mx))
                {
                    if ((pHo->hoFlags & HOF_DESTROYED) == 0)
                    {
                        rhCurParam[1] = pHo->hoOi;
