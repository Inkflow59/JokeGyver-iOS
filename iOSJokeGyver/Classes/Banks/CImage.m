...
// Comment the following define to use internal APIes to load images instead of turbojoeg lib
#define TURBO_JPEG_LIB

// Uncomment the following define to allow profiling loading time
//#define PROFILE_LOAD
...
-(void)load:(CFile*)f
{
    ...
    if(format < JPEG)
    {
        ...
        data = (unsigned int*)malloc(dataLength);
    ...
    {
        NSData* imageData = [file getSubData:chunkSize];
        unsigned int *rawData = nil;

#ifdef TURBO_JPEG_LIB
        
        int width, height;
        loadJPEGFromData((unsigned char*)imageData.bytes, (unsigned long)imageData.length, &width, &height, &rawData);
#else
        UIImage* imageToLoad = [UIImage imageWithData:imageData];
        
        CGImageRef imageRef = [imageToLoad CGImage];
        size_t width = CGImageGetWidth(imageRef);
        size_t height = CGImageGetHeight(imageRef);
        
        CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
        rawData = (unsigned int*)calloc(width*height, sizeof(unsigned int));
    ...
    time_consumed = (double)(end - start) / CLOCKS_PER_SEC * 1000.0;
    NSLog(@"Consume loading: %f type JPEG %@", time_consumed, (format == JPEG ? @"yes" :@"no"));
#endif
}
...
-(void)reload
{
	if (data!=nil)
		return;
#ifdef PROFILE_LOAD
    static double time_consumed = 0, time_middle = 0;
    clock_t start, end, middle;
    start = clock();
    
    NSLog(@"reload image handle: %i", handle);
#endif
    if(format < JPEG)
    {
    ...
    time_middle = (double)(middle - start) / CLOCKS_PER_SEC * 1000.0;
    time_consumed = (double)(end - start) / CLOCKS_PER_SEC * 1000.0;
    NSLog(@"Consume middle: %f reloading: %f type JPEG %@", time_middle, time_consumed, (format == JPEG ? @"yes" :@"no"));
#endif
}
...
-(CMask*)getMask:(int)nFlags withAngle:(float)angle andScaleX:(double)scaleX andScaleY:(double)scaleY
{
	if ((nFlags & GCMF_PLATFORM) == 0)
	{
		if (mask==nil)
		{
			...
			if (opaqueMask)
				[mask createOpaqueMask:self withFlags:nFlags];
			else
	...
	else
	{
		if (maskPlatform == nil)
		{
			...
			if (opaqueMask)
				[maskPlatform createOpaqueMask:self withFlags:nFlags];
			else
...
{
	int maskPitchW = (maskWidth+7)/16;
	[self reload];
	unsigned char* datab = (unsigned char*)data;
	int ymax = MIN(height,maskHeight);
	for(int y=0; y<ymax; y++)
	{
		int xmax = MIN(width,maskWidth);
		for(int x=0; x<xmax; x++)
		{
			short m = mask[y * maskPitchW + x/16];
			int bitm = (0x8000 >> (x & 15));
			int offset = lineWidth * y + x * bytesPrPixel;
			switch(format) {
				case RGB888:
					*((int*)(datab+offset)) &= 0xFF000000;
					if ( m & bitm )
						*((int*)(datab+offset)) |= 0x00FFFFFF;
					break;
				case RGBA8888:
					*((int*)(datab+offset)) = 0xFF000000;
					if ( m & bitm )
						*((int*)(datab+offset)) = 0xFFFFFFFF;
					break;
				case RGBA4444:
				case RGBA5551:
				case RGB565:
					*((short*)(datab+offset)) = 0;
					if ( m & bitm )
						*((short*)(datab+offset)) = 0xFFFF;
					break;
			}
		}
	}
...
-(void)copyImage:(CImage*)image
{
    ...
    bytesPrPixel=image->bytesPrPixel;
    format=image->format;
    int bLineWidth = width*bytesPrPixel;
    switch (format)
    {
        case RGBA4444:
        case RGBA5551:
        case RGB565:
            bLineWidth = (width + (width % 2))*2;
            break;
        case RGB888:
        case JPEG:
            bLineWidth = (bLineWidth+3) & ~3;
            break;
    }
    ...
    unsigned int datasize = bLineWidth*image->height;
	data=(unsigned int*)malloc(datasize);
    
	int n;
	unsigned int* pSrce=image->data;
	unsigned int* pDest=data;
	for (n=datasize/sizeof(int); n>0; n--)
	{
		*(pDest++)=*(pSrce++);
	}
    int rest = datasize % sizeof(int);
    if(rest != 0)
    {
        unsigned char* pSrce1=(unsigned char*)pSrce;
...
//    NSLog(@"upload texture handle: %i", handle);
	isUploading = YES;
...
{
    return (flags & IMGFLAG_REPLACEDCOLORS) != 0;
}
-(void)setReplacedColorFlag:(bool)flag
{
    if ( flag )
        flags |= IMGFLAG_REPLACEDCOLORS;
    else
        flags &= ~IMGFLAG_REPLACEDCOLORS;
}
