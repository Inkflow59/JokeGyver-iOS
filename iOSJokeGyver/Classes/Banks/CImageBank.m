...
-(void)preLoad
{
    ...
    useCountCopy=(short*)malloc(nHandlesReel*sizeof(short));
	[self resetToLoad];
	handleToIndex=nil;
...
-(void)copyUseCount
{
    ...
    for (n=0; n<nHandlesReel; n++)
    {
        useCountCopy[n] = useCount[n];
    }
//    memcpy(&useCountCopy[0], &useCount[0], nHandlesReel*sizeof(short));
}
-(void)load
{
	int n;
...
-(void)loadUnloaded
{
    ...
    for (h=0; h<nHandlesReel; h++)
    {
        if (useCount[h] > useCountCopy[h])
        {
            if(handleToIndex[h] == -1 || images[handleToIndex[h]] == nil)
            {
                //using same code as in other parts
                int i;
                int iFound=-1;
                for (i=0; i<nImages; i++)
                {
                ...
                handleToIndex[h]=(short)iFound;
                images[iFound]=[[CImage alloc] initWithApp:runApp];

                [runApp->file seek:offsetsToImages[h]];
                [images[iFound] load:runApp->file];
                images[iFound]->isPrunable = true;

                useCountCopy[h]=useCount[h];
                images[iFound]->useCount=useCount[h];
                
                [images[iFound] reUploadTexture];
            ...
            else
            {
                ...
                CImage* image = images[handleToIndex[h]];
                useCountCopy[h]=useCount[h];
                image->useCount=useCount[h];
                
                [image reUploadTexture];
...
-(void)unloadLoaded
{
    ...
    for (h=0; h<nHandlesReel; h++)
    {
        int discardCount = useCount[h] - useCountCopy[h];
        if (discardCount != 0)
        {
            useCount[h] = useCountCopy[h] - discardCount;
            useCountCopy[h] = useCount[h];
            if (useCount[h] == 0 && images!=nil && handleToIndex[h]!=-1 && images[handleToIndex[h]]!=nil)
            {
                //NSLog(@"Discarded handle:%i", h);
                runApp->renderer->removeTexture(images[handleToIndex[h]], NO);
                [self delImage:h];
            }
            count++;
        }
    }
    //NSLog(@"Discarded %i images", count);
}
...
-(void)loadImageByHandle:(short)handle
{
    if (offsetsToImages[handle]!=0)
    {
        if ([self getImageFromHandle:handle]==nil)
        {
            // Cherche une image libre
            int i;
            int iFound=-1;
            ...
            // Ajoute la nouvelle image
            handleToIndex[handle]=(short)iFound;
            images[iFound]=[[CImage alloc] initWithApp:runApp];
            images[iFound]->useCount=1;
            [runApp->file seek:offsetsToImages[handle]];
            [images[iFound] load:runApp->file];
            images[iFound]->isPrunable = false;
        }
        else
        {
            [self getImageFromHandle:handle]->useCount++;
        }
    }
}
...
-(ImageInfo)getImageInfoEx:(short)nImage withAngle:(float)nAngle andScaleX:(float)fScaleX andScaleY:(float)fScaleY
{
    ...
    if ( ptei == nil )
    {
        if(offsetsToImages[nImage] != 0 && useCount[nImage] != 0)
        {
            pIfo.isFound = YES;
            [self loadImageByHandle:nImage];
            ptei = [self getImageFromHandle:nImage];
            if ( ptei == nil )
            {
                pIfo.isFound = NO;
