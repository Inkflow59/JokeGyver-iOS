...
-(CSprite*)addOwnerDrawSprite:(int)x1 withY1:(int)y1 andX2:(int)x2 andY2:(int)y2 andLayer:(short)wLayer andZOrder:(int)nZOrder andBackColor:(int)backSpr andFlags:(int)sFlags andObject:(CObject*)extraInfo andDrawable:(id)sprProc
{
    ...
    if(extraInfo != nil && extraInfo->ros != nil)
    {
        ptSpr->sprEffect = extraInfo->ros->rsEffect;
        ptSpr->sprEffectParam = extraInfo->ros->rsEffectParam;
        ptSpr->sprEffectShader = extraInfo->ros->rsEffectShader;
    }
    else
    {
        ptSpr->sprEffect = EFFECTFLAG_TRANSPARENT; // Adding Blend
        ptSpr->sprEffectParam = 0;
        ptSpr->sprEffectShader = -1;
    }
	ptSpr->sprScaleX = ptSpr->sprTempScaleX = ptSpr->sprScaleY = ptSpr->sprTempScaleY = 1.0f;
...
-(CSprite*)modifSpriteEffect:(CSprite*)ptSpr withInkEffect:(int)effect andInkEffectParam:(int)effectParam andEffectShader:(int)effectShader 
{
    ...
    ptSpr->sprEffectShader = effectShader;
    
	// reafficher sprite
	ptSpr->sprFlags |= SF_REAF;

	if ((ptSpr->sprEffect & EFFECT_MASK) == BOP_BLEND)
	{
		if(ptSpr->sprEffectParam == 128)
		{
	...
	if ((ptSpr->sprEffect & EFFECT_MASK) == BOP_RGBAFILTER)
	{
		if(((ptSpr->sprEffectParam & 0xFF000000) >> 24) == 0)
		{
            ptSpr->sprFlags |= SF_DISABLED;
		}
		else
...
-(void)pasteSpriteEffect:(CRenderer*)renderer withImage:(short)iNum andX:(int)iX andY:(int)iY andFlags:(int)flags andInkEffect:(int)effect andInkEffectParam:(int)effectParam andEffectShader:(int)effectShader
{
    ...
    if(effectShader != - 1)
    {
        renderer->setEffectShader(effectShader);
        renderer->updateVariable1f(@"fPixelWidth",1.0f/(float)ptei->width);
        renderer->updateVariable1f(@"fPixelHeight",1.0f/(float)ptei->height);
    }
    else
        renderer->removeEffectShader();
	// Blit
...
//	uint64_t start = mach_absolute_time();
	...
	for (ptSpr = ptFirstSpr; ptSpr != nil; ptSpr = ptSpr->objNext)
	{
		...
		if(ptSpr->sprLayer/2 != currentLayer)
		{
        ...
        {
            if (ptSpr->sprEffectShader != currentShader)
            {
                currentShader = ptSpr->sprEffectShader;
                if(currentShader != - 1)
                {
                    renderer->setEffectShader(ptSpr->sprEffectShader);
                    if((ptSpr->sprFlags & SF_REAF) == SF_REAF)
                    {
                        renderer->updateVariable1f(@"fPixelWidth",1.0f/(float)(ptSpr->sprX2 - ptSpr->sprX1));
                        renderer->updateVariable1f(@"fPixelHeight",1.0f/(float)(ptSpr->sprY2 - ptSpr->sprY1));
                    }
                }
                else
                    renderer->removeEffectShader();
...
-(CSprite*)spriteCol_TestPoint:(CSprite*)firstSpr withLayer:(short)nLayer andX:(int)xp andY:(int)yp andFlags:(int)dwFlags
{
	...
	for (; ptSpr != nil; ptSpr = ptSpr->objNext)
	{
		...
		if (bEvenNoCol || (ptSpr->sprFlags & SF_RAMBO) != 0)
		{
            ...
            int lx = (xp) + round(windowX*layer->xCoef) + layer->x ;
            int ly = (yp) + round(windowY*layer->yCoef) + layer->y ;
            /*
            NSLog(@"Window: %d, %d and Parent: %d %d", windowX, windowY, app->parentX, app->parentY);
            CObject *obj = ptSpr->sprExtraInfo;
            CObjInfo *XInfo = obj->hoOiList;
            NSString *s = XInfo->oilName;
            NSLog(@"name: %@, mouse: %d, %d and conversion: %d, %d located from: %d, %d inside: %@ and the LayerX is: %d",
                    s, xp, yp, lx, ly, ptSpr->sprX1, ptSpr->sprY1, (lx >= ptSpr->sprX1 && lx < ptSpr->sprX2 && ly >= ptSpr->sprY1 && ly < ptSpr->sprY2 ?@"yes":@"no"), layer->x);
            */
...
-(BOOL)spriteCol_TestPointOne:(CSprite*)firstSpr withLayer:(short)nLayer andX:(int)xp andY:(int)yp andFlags:(int)dwFlags
{
    ...
    if (bEvenNoCol || (ptSpr->sprFlags & SF_RAMBO) != 0)
    {
        ...
        CObjInfo *XInfo = obj->hoOiList;
        NSString *s = XInfo->oilName;
        NSLog(@"name: %@, mouse: %d, %d and calc: %d, %d parent: %d, %d layer pos: %d, %d sprite at: %d, %d inside: %@ and the layerX is: %d",
              s, xp, yp, lx, ly, app->parentX, app->parentY, layer->x, layer->y, ptSpr->sprX1, ptSpr->sprY1, (lx >= ptSpr->sprX1 && lx < ptSpr->sprX2 && ly >= ptSpr->sprY1 && ly < ptSpr->sprY2 ?@"yes":@"no"), layer->x);
        */
        if (lx >= ptSpr->sprX1 && lx < ptSpr->sprX2 && ly >= ptSpr->sprY1 && ly < ptSpr->sprY2)
        {
            int nGetColMaskFlag = GCMF_OBSTACLE;
