...
-(void)draw:(CRenderer*)renderer
{
	...
	if(toDraw != nil)
	{
        if(sprEffectShader != - 1)
        {
            renderer->setEffectShader(sprEffectShader);
            if((sprFlags & SF_REAF) == SF_REAF)
            {
                renderer->updateVariable1f(@"fPixelWidth",1.0f/(float)(sprX2 - sprX1));
                renderer->updateVariable1f(@"fPixelHeight",1.0f/(float)(sprY2 - sprY1));
            }
        }
        else
        {
