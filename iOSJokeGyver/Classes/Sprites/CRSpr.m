...
-(id)initWithHO:(CObject*)ho andOC:(CObjectCommon*)ocPtr andCOB:(CCreateObjectInfo*)cobPtr
{
    ...
    rsEffectShader = -1;
    if((rsEffect & BOP_MASK) == BOP_EFFECTEX)
    {
        if(rsEffectShader == -1)
            rsEffectShader = [hoPtr checkOrCreateEffectIfNeededByIndex:hoPtr->hoOiList->oilIndexEffect andEffectParam:rsEffectParam];
    }
...
-(void)handle
{
	...
	if ((rsFlags&RSFLAG_SLEEPING)==0)
	{
        ...
        {
            // Build 285.1 Destroy the object if too far and NEVERKILL is not set
            if ( (hoPtr->hoOEFlags&OEFLAG_NEVERKILL) == 0 && (rhPtr->rhApp->hdr2Options & AH2OPT_DESTROYIFNOINACTIVATE) != 0 )
            {
...
-(void)modifRoutine
{
    ...
    if(hoPtr->hoEffect != nil)
    {
        [hoPtr->hoEffect refreshParamSurface];
    }
...
-(BOOL)createSprite:(CSprite*)pSprBefore
{
	...
	if ((hoPtr->hoOEFlags&OEFLAG_ANIMATIONS)!=0)
	{
		...
		if (pSpr!=nil)
		{
            ...
            hoPtr->roc->rcChanged = 1;

			[spriteGen modifSpriteEffect:pSpr withInkEffect:rsEffect andInkEffectParam:rsEffectParam andEffectShader:rsEffectShader];
