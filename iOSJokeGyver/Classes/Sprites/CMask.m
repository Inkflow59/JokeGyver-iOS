...
-(void)createOpaqueMask:(CImage*)img withFlags:(int)nFlags
{
	...
	int hmax = height;
	if ((nFlags & GCMF_PLATFORM) != 0 && hmax > HEIGHT_PLATFORM)
		hmax = HEIGHT_PLATFORM;
	
	int wmax = maskWidth;
	unsigned short lastWord = 0;
	if ((width & 15) != 0)
	{
		lastWord = (0xFFFF << (16 - (width & 15)));
		wmax--;
	}
		
	for (y = 0; y < hmax; y++)
	{
		int maskOffset = y * maskWidth;
		for (x = 0; x < wmax; x++)
			mask[maskOffset++] = 0xFFFF;
		if (lastWord != 0)
			mask[maskOffset] = lastWord;
	}
}
...
-(BOOL)testRect:(int)yBase1 withX:(int)xx andY:(int)yy andWidth:(int)w andHeight:(int)h
{
	...
	for (y = 0; y < yCount; y++)
	{
		...
		{
			case 1:
				m = (short) (lMask[x1 & 15] & rMask[((x2 - 1) & 15) + 1]);
