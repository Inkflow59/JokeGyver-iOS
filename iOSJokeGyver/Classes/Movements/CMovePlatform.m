...
-(void)move
{
	...
	if ((MP_Type == MPTYPE_WALK || MP_Type == MPTYPE_CLIMB) && MP_NoJump == NO)
	{
		...
		if (bJump)
		{
            MP_YSpeed = -MP_Jump * 256; //<< 8;                  // JUMP_COEF;
			MP_Type = MPTYPE_JUMP;
		}
	}
	switch (MP_Type)
...
-(void)stop
{
	...
	// ------------------------------------
	switch ((int)(hoPtr->hoAdRunHeader->rhEvtProg->rhCurCode & 0xFFFF0000))
	{
		case (-12 * 65536):         // CNDL_EXTOUTPLAYFIELD:
			...
			MP_JumpStopped = 0;
			return;
		case (-13 * 65536):	    // CNDL_EXTCOLBACK:
		case (-14 * 65536):	    // CNDL_EXTCOLLISION:
		{
			MP_NoJump = NO;
...
-(void)mpHandle_Background
{
	...
	[hoPtr->hoAdRunHeader->rhEvtProg handle_Event:hoPtr withCode:(-13 * 65536) | (((int) hoPtr->hoType) & 0xFFFF)];	    // CNDL_EXTCOLBACK
}
