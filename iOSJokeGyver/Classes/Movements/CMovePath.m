...
-(void)mtMessages
{
	if (hoPtr->hoMark1==hoPtr->hoAdRunHeader->rhLoopCount)
	{
		...
		[hoPtr->hoAdRunHeader->rhEvtProg handle_Event:hoPtr withCode:(-20*65536)|(((int)hoPtr->hoType)&0xFFFF) ];	    // CNDL_EXTPATHNODE
		[hoPtr->hoAdRunHeader->rhEvtProg handle_Event:hoPtr withCode:(-35*65536)|(((int)hoPtr->hoType)&0xFFFF) ];	    // CNDL_EXTPATHNODENAME
	}
	if (hoPtr->hoMark2==hoPtr->hoAdRunHeader->rhLoopCount)
	{
		hoPtr->hoAdRunHeader->rhEvtProg->rhCurParam[0]=0;
	    [hoPtr->hoAdRunHeader->rhEvtProg handle_Event:hoPtr withCode:(-21*65536)|(((int)hoPtr->hoType)&0xFFFF) ];   // CNDL_EXTENDPATH
	}
}
...
-(void)reverse
{
	if (rmStopSpeed==0)
	{
		...
		else
		{
			...
			MT_Calculs=(calcul*65536)|(MT_Calculs&0xFFFF);
		}
	}       
}
