...
void CRunBox2DBouncingBall::Initialize(LPHO pHo, CFile* file)
{
    ...
    [file skipBytes:1];
    m_pHo->roc->rcDir=(int)[m_movement dirAtStart:[file readAInt]];
    m_angle=(float)(((float)m_pHo->roc->rcDir*180.0)/16.0);
...
BOOL CRunBox2DBouncingBall::CreateBody(LPHO pHo)
{
    ...
    m_base->pBodySetAngle(m_base, m_mBase->m_body, m_angle);
...
void CRunBox2DBouncingBall::CreateFixture(LPHO pHo)
{
	...
	switch(m_shape)
	{
        case 0:
            ...
            break;
        case 1:
            m_fixture=m_base->pBodyCreateCircleFixture(m_base, m_mBase->m_body, m_mBase, m_pHo->hoX, m_pHo->hoY, (int)((m_imgWidth + m_imgHeight + 2)/4 * (m_scaleX + m_scaleY) / 2), m_density, m_friction, m_restitution);
            break;
        case 2:
...
void CRunBox2DBouncingBall::SetTheAngle(float angle)
{
    ...
    {
        m_pHo->roc->rcDir=(int)round(angle/11.25);
...
void CRunBox2DBouncingBall::Stop(LPHO pHo, BOOL bCurrent)
{
	...
	if (m_mBase->m_eventCount!=pHo->hoAdRunHeader->rh4EventCount)
	{
		if (m_angleMoving == MOVING)
		{
			b2Vec2 v = m_mBase->m_body->GetLinearVelocity();
			float angle=(float)(atan2(v.y, v.x)*180.0/b2_pi);
			m_angleMoving=angle;
		}
...
{
	if (m_flags&BBFLAG_ROTATE)
		return (int)round(m_mBase->m_currentAngle/11.25f) % 32;
	else
