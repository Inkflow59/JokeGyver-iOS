...
BOOL CRunBox2DPlatform::CreateBody(LPHO pHo)
{
	...
	m_mBase->m_currentAngle = 0.0;
	if (pHo->roc->rcDir == 16)
		m_mBase->m_currentAngle = 180.0;
	pHo->roc->rcSpeed=0;
...
BOOL CRunBox2DPlatform::Move(LPHO pHo)
{
	...
	if (m_jump == 0)
		m_jumpCounter = m_jumps;
			...
			if (!flag)
			{
				if (ladderDir == 0 && (m_jump==0 || ((m_flags&MPFLAG_JUMPSTOPXIFNOINPUT)!=0 && (joyDir&12) == 0)))
				{
					if (m_currentSpeed>0)
					{
						m_currentSpeed-=m_deceleration;
