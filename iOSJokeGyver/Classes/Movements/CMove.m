...
-(BOOL)newMake_Move:(int)speed withDir:(int)angle
{
    ...
    {
        x = (int64_t)hoPtr->hoX * 65536 | (hoPtr->hoCalculX & 0x0000FFFF);
        y = (int64_t)hoPtr->hoY * 65536 | (hoPtr->hoCalculY & 0x0000FFFF);
...
-(void)mv_Approach:(BOOL)bStickToObjects
{
    ...
    BOOL flag = NO;
    constexpr uint SW_CNDL_EXTOUTPLAYFIELD = -12 * 65536;
    constexpr uint SW_CNDL_EXTCOLBACK = -13 * 65536;
    constexpr uint SW_CNDL_EXTCOLLISION = -14 * 65536;

    switch (hoPtr->hoAdRunHeader->rhEvtProg->rhCurCode & 0xFFFF0000)
    {
        case (SW_CNDL_EXTOUTPLAYFIELD):         // CNDL_EXTOUTPLAYFIELD:
            {
                // --------------------------------------------------------------------------------
                // Sortie du terrain...
                // --------------------------------------------------------------------------------
            ...
            return;
        case (SW_CNDL_EXTCOLBACK):        // CNDL_EXTCOLBACK:
        case (SW_CNDL_EXTCOLLISION):        // CNDL_EXTCOLLISION:
            {
