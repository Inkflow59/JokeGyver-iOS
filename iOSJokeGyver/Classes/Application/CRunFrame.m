...
-(void)clearSprites
{
	...
	for (n=0; n<nLayers; n++)
	{
		CLayer* pLayer = layers[n];
		if (pLayer != nil && pLayer->pBkd2 != nil)
		{
			int size = [pLayer->pBkd2 size];
			for (int i = 0; i < size; i++)
			{
				CBkd2* pbkd = (CBkd2*)[pLayer->pBkd2 get:i];
				if (pbkd != nil)
				{
					[pbkd clearSprites];
				}
	...
	while (chk->chID != CHUNK_LAST)	
	{
		...
		switch (chk->chID)
		{
			case CHUNK_FRAMEHEADER:
				...
				{
                    if((app->gaFlags & GA_STRETCH) != 0
                       && (app->widthSetting > leWidth || app->heightSetting > leHeight))
                    {
                        app->gaCxWin = leWidth;
                        app->gaCyWin = leHeight;
                        leEditWinWidth = leWidth;
                        leEditWinHeight = leHeight;
                    }
                    else
                    {
			...
			case CHUNK_FRAMELAYERS:
				[self loadLayers];
				break;
            case CHUNK_FRAMELAYEREFFECTS:
                [self loadLayerEffects];
                break;
...
//				[evtProg relocatePath:app];
            ...
            case CHUNK_EFFECTS:
            {
                effect = [app->file readAInt];
                effectParam = [app->file readAInt];
                effectIndex = [app->file readAInt];
                effectNParams = [app->file readAInt];
                
                int number_params = effectNParams;
                if(number_params > 0)
                {
                    effectData = (int*)malloc(number_params*sizeof(int));
                    for(int i=0; i<number_params; i++)
                    {
                        effectData[i] = [app->file readAInt];
                    }
                }
                break;
            }
	...
	// Change obstacle background objects in box mode if windows-like collisions
	if ((app->hdr2Options2 & AH2OPT2_WINDOWSLIKECOLLISIONS) != 0)
	{
		for (n=0; n<app->OIList->oiMaxIndex; n++)
		{
			COI* poi = app->OIList->ois[n];
			if ( poi!=NULL && poi->oiOC!=NULL && poi->oiType == OBJ_BKD)
			{
				COC* poc = poi->oiOC;
				int v = poc->ocObstacleType;
				if ( poc->ocColMode != 0 && (v==OBSTACLE_SOLID || v==OBSTACLE_PLATFORM) )
				{
					poc->ocColMode = 0;
					short imgNum = ((COCBackground*) poc)->ocImage;
					CImage* img = [app->imageBank getImageFromHandle:imgNum];
					if (img != nil)
...
{
    int start_block = (int)[app->file getFilePointer];
    int n;

    for (n=0; n<nLayers; n++)
    {
        layers[n]->effect = [app->file readAInt];
        layers[n]->effectParam = [app->file readAInt];
        layers[n]->effectIndex = [app->file readAInt];
        layers[n]->effectNParams = [app->file readAInt];
        layers[n]->effectPOffset = start_block + [app->file readAInt];
    }

    for (n=0; n<nLayers; n++)
    {
        int number_params = layers[n]->effectNParams;

        if(number_params > 0)
        {
            layers[n]->effectData = (int*)malloc(number_params*sizeof(int));

            [app->file seek:layers[n]->effectPOffset];

            for (int i = 0; i < number_params; i++)
            {
                layers[n]->effectData[i] = [app->file readAInt];
            }
        }
    }
}
    ...
    if(![app->effectBank isEmpty])
    {
        if (effectEx == nil)
        {
            ...
            if ([effectEx initializeByIndex:effectIndex withEffectParam:effectParam])
            {
                ...
                frameInkEffectShader = [effectEx getIndexShader];
            }
            else
                frameInkEffectShader =  -1;
        }
