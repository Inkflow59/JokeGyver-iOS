...
//Commented following define if you want the texture ram garbagge collector to stop running, best when using big textures
#define TEXTURE_RAM_GC
...
//                                          kEAGLDrawablePropertyColorFormat     : kEAGLColorFormatRGBA8};
        ...
        //Attempt to create ES3 renderer, if not possible create an ES2 renderer
...
#else
            displayLink.preferredFramesPerSecond=pRunApp->gaFrameRate;
#endif
...
#else
        frameTime = 1.0/(double)displayLink.preferredFramesPerSecond;
#endif
		bank -= frameTime;
		if(bank > 0)
			return;
...
//Routine that runs every 3*15 seconds to find any textures that aren't in use during that 15 second window
-(void)cleanEntry
{
...
- (void)layoutSubviews
{
    renderer->windowSize = CGSizeMake(appRect.size.width, appRect.size.height);
