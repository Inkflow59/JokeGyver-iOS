...
-(id)initAsSubApp:(CRunApp*)app
{
	if(self = [super init])
	{
        ...
        mainViewController = app->mainViewController;
        [file setFilePointer:0];
	}
	return self;
}	
...
{	
    ...
    effectBank= [[CEffectBank alloc] initWithApp:self];
    
    if(![NSClassFromString(@"CRuniOSPlus") class])

        	enable_ext_sounds = NO;

    	else
 
        	enable_ext_sounds = YES;
	...
	if(parentApp == nil){
        ...
        [ALPlayer setAllowOtherAppSounds:enable_ext_sounds];
	...
	while (chk->chID != CHUNK_LAST)
	{
		...
		switch (chk->chID)
		{
			...
			case CHUNK_FRAMEITEMS_2:
                ...
                if ((hdr2Options2 & AH2OPT2_IOS_ENABLELOADONCALL) == 0 )
                {
                    COI* oi = nil;
                    for (oi = [OIList getFirstOI]; oi != nil; oi = [OIList getNextOI])
                        oi->oiFlags &= ~OIF_LOADONCALL;
                }
                break;
...
-(void) UpdateViewport
{
    ...
    CGSize app = CGSizeMake(widthSetting, heightSetting);
    
    double appAspect = app.width/app.height;
    ...
    switch(viewMode)
    {
        ...
        default:
        {
            gaCxWin = widthSetting;
            gaCyWin = heightSetting;
            break;
        }
    }
...
-(void)createDisplay
{
    ...
    if((gaNewFlags & GANF_DONOTCENTERFRAME) != 0)
    {
        xOffset=0;
        yOffset=0;
    }
    else
    {
        ...
        xOffset = sxView/2 - cxLog/2;
        yOffset = syView/2 - cyLog/2;
    }
}		
...
-(BOOL)playApplication:(BOOL)bOnlyRestartApp
{
	...
	while(cApp != nil && cApp->parentApp != nil)
	{
		parentX += (cApp->subApp->hoRect.left );
		parentY += (cApp->subApp->hoRect.top );
        CLayer* layer = cApp->subApp->hoAdRunHeader->rhFrame->layers[cApp->subApp->hoLayer];
        
       /*
        if ( cApp->parentApp->run != nil )
        {
            parentX -= cApp->parentApp->run->rhWindowX;
            parentY -= cApp->parentApp->run->rhWindowY;
        }
        */
...
//Update window position for CCCA
...
-(void)endFrame
{
	...
	{
        //Erase Frame Effect
        if((frame->effect & EFFECT_MASK) == BOP_EFFECTEX)
        {
            if(frame->effectEx != nil)
                [frame->effectEx release];
        }
        //Erase layers Effect
        for(int i = 0; i < frame->nLayers; i++)
        {
            CLayer* pLayer = frame->layers[i];
            if((pLayer->effect & EFFECT_MASK) == BOP_EFFECTEX)
            {
                if(pLayer->effectEx != nil)
                    [pLayer->effectEx release];
            }
        }
		// Reset current frame
		currentFrame = -1;
...
-(void)createLayerEffects:(CRunFrame*)frame
{
    int nLayers = frame->nLayers;
    ...
    for(int i = 0; i < nLayers; i++)
    {
        ...
        if((pLayer->effect & EFFECT_MASK) == BOP_EFFECTEX)
        {
            if(pLayer->effectShader != -1)
                [pLayer checkOrCreateEffectIfNeeded:self];
        }
    }
}
...
-(void)loadAppHeader
{
	...
	[file skipBytes:4];
    gaCxWin = widthSetting;        // Saving Original Window Width
    gaCyWin = heightSetting;        // Saving Original Window Height
...
-(void)loadAppHeader2
{
	...
	if(hdr2Options&AH2OPT_STATUSLINE)
        ...
        [UIApplication sharedApplication].statusBarOrientation = uiOrientation;
#endif
	[runView layoutSubviews];
}
...
//        NSLog(@"Value at:%d is:%d", n, globalValuesInit[n]);
	}
...
//        NSLog(@"String at:%d is:%@", n, globalStringsInit[n]);
	}
}
...
-(void)mouseMoved:(int)x withY:(int)y
{
    mouseX = x - parentX;
    mouseY = y - parentY;
    //NSLog(@"mouseMoved: %d, %d and parent: %d, %d ",x, y, parentX, parentY);
...
-(void)mouseClicked:(int)numTap
{
    ...
    if (run!=nil)
    {
        CSysEventClick* click=[[[CSysEventClick alloc] initWithClick:numTap] autorelease];
        if(run->rhEvtProg != nil)
            [click execute:run];
        else
            [sysEvents add:click];
    }
...
-(void)positionUIElement:(UIView*)view withObject:(CObject*)ho
{
    ...
    plusX -= run->rhWindowX;
    plusY -= run->rhWindowY;
    //NSLog(@"parentX:  %d, parentY: %d, plusX: %d, plusY: %d",parentX, parentY, plusX, plusY);
...
-(NSData*)loadResourceData:(NSString*)path
{
    ...
    {
        //Support directory
        NSArray *ap_paths = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES);
        NSString *supportDirectory = [ap_paths firstObject];
        if (supportDirectory) {
            NSString* appBundleID = [[NSBundle mainBundle] bundleIdentifier];
            NSString* appSupportDir = [supportDirectory stringByAppendingPathComponent:appBundleID];
            NSString* appsupportPath = [appSupportDir stringByAppendingPathComponent:path];
            if([[NSFileManager defaultManager] fileExistsAtPath:appsupportPath])
                return [NSData dataWithContentsOfFile:appsupportPath];
        }
    }
	...
	if(embeddedFiles != nil)
	{
		...
		for(NSUInteger i=0; i<embLen; ++i)
		{
            ...
            if([path isEqualToString:embFile->path]
                || [embFile->path containsString:path])
                return [embFile open];
		}
	}
	
    if([path containsString:@"assets"])
...
-(CGSize)windowSize
{
    if([UIScreen mainScreen] != nil)
    {
        ...
        switch (actualOrientation) {
            ...
            default:
                portrait = CGSizeMake(screen.width, screen.height);
                break;
        }
    ...
    {
        CGSize portrait = CGSizeMake(0, 0);
        return portrait;
    }
}
...
-(CGSize)screenSize
{
    ...
    switch (actualOrientation) {
        default:
        case ORIENTATION_SENSOR:
            screen = CGSizeMake(screenBounds.width * scale, screenBounds.height * scale);
            break;
        case ORIENTATION_LANDSCAPELEFT:
        case ORIENTATION_LANDSCAPERIGHT:
        case ORIENTATION_AUTOLANDSCAPE:
            CGSize portrait = CGSizeMake(MIN(screenBounds.width, screenBounds.height), MAX(screenBounds.width, screenBounds.height));
            screen = CGSizeMake(portrait.height * scale, portrait.width * scale);
            break;
    }
...
-(BOOL)cndClipboard
{
    ...
    NSString* ret = pasteboard.string;
    if(ret && [ret length] > 0)
        return true;
