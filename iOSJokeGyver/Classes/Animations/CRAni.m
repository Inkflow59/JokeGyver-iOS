...
-(BOOL)animIn:(int)vbl
{
	...
	if (anim != raAnimOn)								//; La meme?
	{
		...
		if (anPtr != raAnimOffset)
		{
			...
			if ( (hoPtr->hoOEFlags & OEFLAG_DONTRESETANIMCOUNTER) == 0 )		// Added for bug http://bugbox.clickteam.com/issues/3421, triggered by fix below...
	    		raAnimCounter = 0;		// Build 284.11, report from Android, fixes bug http://bugbox.clickteam.com/issues/1882
		}
	}
	...
	if (bAngle==NO && raAnimNumberOfFrame == 0 && anim != ANIMID_DISAPPEAR)
	{
			...
			if (frame >= raAnimNumberOfFrame)
			{
				...
				if (raAnimRepeat != 0)					//; On boucle?
				{
					...
					if (raAnimRepeat == 0)
					{
						...
						{
							short image = adPtr->adFrames[raAnimFrame];
                        ...
                        int cond = (-2 * 65536);	    // CNDL_EXTANIMENDOF;
