		...
		while (chk->chID!=CHUNK_LAST)
		{
			...
			switch(chk->chID)
			{
				...
				case 0x4446:
				{
                    ...
                    // CHUNK_OBJECTSEFFECTS
                case 0x4448:
                {
                    ois[index]->oiEffectOffset = [file getFilePointer];
                    if(ois[index]->oiEffectOffset != 0)
                        [ois[index] loadEffectData:file];
                    break;
                }
			}
...
{
    ois[oiHandleToIndex[h]]->oiLoadFlags |= OILF_TORELOAD;
}
