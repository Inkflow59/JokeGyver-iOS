...
-(void)draw:(CRenderer*)renderer
{
	...
	{
		case CTA_ANIM:
			[hoAdRunHeader->spriteGen pasteSpriteEffect:renderer withImage:adCta->frames[rsOldFrame] andX:(int)hoRect.left andY:(int)hoRect.top andFlags:0 andInkEffect:effect andInkEffectParam:effectParam andEffectShader:effectShader];
			break;
		...
		case CTA_DIGITS:
		{
			...
			for (i = 0; i < cachedLength; i++)
			{
                ...
                [hoAdRunHeader->spriteGen pasteSpriteEffect:renderer withImage:img andX:x andY:y andFlags:0 andInkEffect:effect andInkEffectParam:effectParam andEffectShader:effectShader];
				CImage* ifo = [hoAdRunHeader->rhApp->imageBank getImageFromHandle:img];
				x += ifo->width;
			}
			break;
