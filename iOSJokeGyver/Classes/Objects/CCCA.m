...
-(void)handle
{
	...
	if (subApp != nil)
	{
        ...
        //NSLog(@"at x:%d, y%d, and called %d", hoX, hoY, sprite->sprExtraInfo->hoOi);
        
        if (sprite!=nil && ((oldX != hoX || oldY != hoY)
                            || (oldWidth!=hoImgWidth || oldHeight!=hoImgHeight)
                            || (hoOEFlags & OEFLAG_SCROLLINGINDEPENDANT) == 0
                            || (nWindowUpdate != hoAdRunHeader->rhApp->nWindowUpdate)))
         
        {
            //NSLog(@"change requested");
            hoRect.left = hoX;
            hoRect.top = hoY;
           ...
           if (oldWidth!=hoImgWidth || oldHeight!=hoImgHeight)
            {
                ...
                if((odOptions & CCAF_STRETCH) == 0) // Non Stretch = Grow in size and frame dimensions
                {
			...
			[hoAdRunHeader->spriteGen modifOwnerDrawSprite:sprite withX1:hoX andY1:hoY andX2:hoX+hoImgWidth andY2:hoY+hoImgHeight];
...
-(void)spriteDraw:(CRenderer*)renderer withSprite:(CSprite*)spr andImageBank:(CImageBank*)bank andX:(int)x andY:(int)y
{
	if (subApp!=nil && subApp->run!=nil && subApp->frame != nil)
	{
		...
		if ((odOptions & CCAF_STRETCH) != 0)
		{
			[rtt setResampling:((hoAdRunHeader->rhApp->hdr2Options & AH2OPT_ANTIALIASED) != 0)];
		...
		else
		{
            ...
            //NSLog(@"x: %d, y: %d layer Offset X: %d, Y: %d and d X: %d, Y: %d", x, y, layer->xOff, layer->yOff, dx, dy);
            renderer->setClip(x-dx, y-dy, fW, fH);
