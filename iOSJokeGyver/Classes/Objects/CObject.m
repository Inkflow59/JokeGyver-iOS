...
-(int)fixedValue
{
	return (hoCreationId * 65536) + (((int)hoNumber) & 0xFFFF);
}
...
-(int)getX
{
    ...
    return hoX - (round(windowX*layer->xCoef) + layer->x) + windowX;
}
...
-(int)getY
{
    ...
    return hoY - (round(windowY*layer->yCoef) + layer->y) + windowY;
}
-(int)globalToLayerX:(int)X
{
    CLayer* layer = hoAdRunHeader->rhFrame->layers[hoLayer];
    int windowX = hoAdRunHeader->rhWindowX;
    return X- windowX + (round(windowX*layer->xCoef) + layer->x);
}
-(int)globalToLayerY:(int)Y
{
    CLayer* layer = hoAdRunHeader->rhFrame->layers[hoLayer];
    int windowY = hoAdRunHeader->rhWindowY;
    return Y- windowY + (round(windowY*layer->yCoef) + layer->y);
}
-(Vec2i)getPosition
{
	CLayer* layer = hoAdRunHeader->rhFrame->layers[hoLayer];
	int windowX = hoAdRunHeader->rhWindowX;
	int windowY = hoAdRunHeader->rhWindowY;
	return Vec2i((int)(hoX - round(windowX*layer->xCoef) - layer->x + windowX), (int)(hoY - round(windowY*layer->yCoef) - layer->y + windowY));
}
...
-(void)setX:(int)x
{
	...
	CLayer* layer = hoAdRunHeader->rhFrame->layers[hoLayer];
	int windowX = hoAdRunHeader->rhWindowX;
	x = (x-windowX + layer->x) + round(windowX*layer->xCoef);
...
-(void)setY:(int)y
{
	...
	CLayer* layer = hoAdRunHeader->rhFrame->layers[hoLayer];
	int windowY = hoAdRunHeader->rhWindowY;
	y = (y-windowY + layer->y) + round(windowY*layer->yCoef);
...
{
	// Set layer
	CLayer* oldlayer = hoAdRunHeader->rhFrame->layers[hoLayer];
	CLayer* layer = hoAdRunHeader->rhFrame->layers[newLayer];
	hoLayer = newLayer;

	//Alter coordinates based on layer coefficients
	int windowX = hoAdRunHeader->rhWindowX;
	int windowY = hoAdRunHeader->rhWindowY;
	int x = hoX - round(windowX*(oldlayer->xCoef - layer->xCoef)) - oldlayer->x + layer->x;
	int y = hoY - round(windowY*(oldlayer->yCoef - layer->yCoef)) - oldlayer->y + layer->y;
...
{
    CRunApp* app = hoAdRunHeader->rhApp;
    if(![app->effectBank isEmpty])
    {
        if (hoEffect == nil) {

            hoEffect = [[CEffectEx alloc] initWithApp:app];
            if ([hoEffect initializeByIndex:index withEffectParam:rgba]) // Needs a handle (from objects)??
            {
                [self fillEffectData];
                return [hoEffect getIndexShader];
            } else
                return -1;
...
-(int)checkOrCreateEffectIfNeededByName:(NSString*)name andEffectParam:(int)rgba
{
    ...
    if(![app->effectBank isEmpty])
    {
        if (hoEffect == nil) {
            ...
            hoEffect = [[CEffectEx alloc] initWithApp:app];
            if ([hoEffect initializeByName:name withEffectParam:rgba])
                return [hoEffect getIndexShader];
            else
                return -1;

        } else if ([hoEffect->name containsString:name])
            return [hoEffect getIndexShader];
        else {
            [hoEffect removeShader];
            [hoEffect release];
...
-(void)fillEffectData
{
    COIList* list = hoAdRunHeader->rhApp->OIList;
    COI* oiPtr = [list getOIFromHandle:hoOi];
    if(oiPtr->oiEffectOffset != 0 && hoEffect != nil && oiPtr->oiEffectData != nil)
       [hoEffect setEffectData:oiPtr->oiEffectData];
