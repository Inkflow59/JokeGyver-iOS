...
// To Use font descent calculation when some fonts require it,
// please uncomment the following define, by removing / chars.
//#define USE_FONT_DESCENT
...
{
    return (a&0xFF)*16777216 | (b&0xFF)*65536 | (g&0xFF)*256 | (r&0xFF);
}
...
{
    return (a&0xFF)*16777216 | (r&0xFF)*65536 | (g&0xFF)*256 | (b&0xFF);
}
...
int nextPOT(int val, int max)
{
    int pot = 8;
    while (pot<val && pot<max)
        pot *= 2;
    return pot;
}
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
#ifdef USE_FONT_DESCENT
    paragraphStyle.minimumLineHeight = f.pointSize;
    paragraphStyle.maximumLineHeight = f.pointSize;
#endif
    if((flags & DT_SINGLELINE) != 0)
    {
        if((flags & DT_ENDELLIPSIS) != 0)
            paragraphStyle.lineBreakMode = NSLineBreakByTruncatingTail;
        else if ((flags & DT_PATHELLIPSIS) != 0)
    ...
    if ([s respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)])
    {
        ...
        if([space respondsToSelector:@selector(sizeWithAttributes:)])
            ...
            size = [s sizeWithFSFont:f constrainedToSize:CGSizeMake(rectWidth, CGFLOAT_MAX) lineBreakMode:(NSLineBreakMode)0];
        size = CGSizeMake(size.width, floor(size.height));
        
#ifdef USE_FONT_DESCENT
        rectSize = [s boundingRectWithSize:CGSizeMake(rectWidth, CGFLOAT_MAX) options:NSStringDrawingUsesDeviceMetrics|NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:f, NSParagraphStyleAttributeName:paragraphStyle} context:nil].size;
#else
        ...
        size=[@" " sizeWithFSFont:f];
        rectSize = [s sizeWithFSFont:f constrainedToSize:CGSizeMake(rectWidth, CGFLOAT_MAX) lineBreakMode:(NSLineBreakMode)0];
...
//    NSLog(@"Fonties lineheight: %f, pointSize: %f, asc: %f, desc: %f, range: %f and size: %f", f.lineHeight, f.pointSize, f.ascender, f.descender, (f.ascender - f.descender), size.height);
#ifdef USE_FONT_DESCENT
    int hLine=f.lineHeight;
#else
    ...
    {
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 80000
        paragraphStyle.alignment = (NSTextAlignment)hAlignment;
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            [s drawInRect:CGRectMake(rc.left, y, rectWidth, MAX(rectHeight, rectSize.height)) withFont:f lineBreakMode:(NSLineBreakMode)0 alignment:(NSTextAlignment)hAlignment];
#pragma clang diagnostic pop
        }
#else
...
+(UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize
{
    ...
    if(image.size.width >= image.size.height)
        newSize.height = (int)((float)(newSize.width*image.size.height)/image.size.width);
    else
        newSize.width = (int)((float)(newSize.height*image.size.width)/image.size.height);
