...
void main()
{
    ...
    if(pDir != 0)
    {
        float ScreenX = 1.0 + sin(((1.0-textureCoordinate.y)*WaveIncrement+Offset)*delta)*Zoom-Zoom;
        posTex = textureCoordinate * vec2(ScreenX, 1.0) + vec2((1.0-ScreenX)/2.0, 0.0);
        color = texture2D(imgTexture, posTex);
    }
    else
    {
        float ScreenY = 1.0 + sin((textureCoordinate.x*WaveIncrement+Offset)*delta)*Zoom-Zoom;
